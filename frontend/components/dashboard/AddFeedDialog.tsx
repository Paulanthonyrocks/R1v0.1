    ...
    return (
        <>
            <Button 
                ...
                variant="outline"
                className={cn(
                    "bg-lcd-text/10 border-lcd-text text-lcd-text hover:bg-lcd-text hover:text-lcd-bg rounded-none uppercase font-bold transition-all",
                    open && "pointer-events-none opacity-50"
                )}
                data-add-feed-trigger
                onClick={() => {
            ...
            <Dialog open={open} onOpenChange={setOpen} modal={true}>
                <DialogContent 
                    ...
                    onPointerDownOutside={(e) => {
                        // Prevent the dialog from closing if the user clicks the trigger button area
                        if (e.target instanceof Element && e.target.closest('[data-add-feed-trigger]')) {
                            e.preventDefault();
                        }
                    }}
                >
