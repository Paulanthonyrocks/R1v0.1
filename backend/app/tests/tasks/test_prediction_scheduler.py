...
class TestPredictionScheduler(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_load_monitored_locations_uses_default_when_no_priority(
        ...
        mock_randint.assert_called_once_with(
            1, len(self.original_hardcoded_locations_with_names)
        )
        ...
        self.scheduler.logger.info.assert_any_call(
            "No priority locations set, using default/random locations for prediction."
        )
        self.scheduler.logger.info.assert_any_call(
            f"Dynamically selected 2 default locations: {[f'{loc.name} ({loc.latitude},{loc.longitude})' for loc in sample_subset]}"
        )
    ...
    async def test_predict_and_notify_high_likelihood(
        ...
        self.mock_analytics_service.predict_incident_likelihood.return_value.set_result(
            mock_prediction_result
        )
        ...
        self.mock_analytics_service._connection_manager.broadcast = (
            MagicMock(return_value=asyncio.Future())
        )
        self.mock_analytics_service._connection_manager.broadcast.return_value.set_result(
            None
        )
        ...
        self.mock_analytics_service._connection_manager.broadcast.assert_awaited_once()

        # Check the details of the broadcast message
        args, kwargs = (
            self.mock_analytics_service._connection_manager.broadcast.call_args
        )
        ...
        self.assertEqual(
            notification_payload.details["autonomous_action"], mock_action_string
        )
        self.assertEqual(
            notification_payload.details["likelihood_percent"],
            mock_prediction_result["likelihood_score_percent"],
        )
        self.assertEqual(
            notification_payload.details["location"], sample_location.model_dump()
        )
    ...
    async def test_predict_and_notify_low_likelihood(self, mock_determine_actions):
        ...
        self.mock_analytics_service._connection_manager.broadcast.assert_not_awaited()  # Notification should not be sent
...
async def test_run_loop_uses_priority_then_default(self):
    ...
    with (
        patch(
            "app.tasks.prediction_scheduler.random.sample", return_value=default_locs
        ),
    ...
    self.assertTrue(
        self.scheduler.logger.info.call_args_list[-2][0][0].startswith(
            "No priority locations set"
        )
    )  # -2 because last is dynamic selected
    self.assertTrue(
        self.scheduler.logger.info.call_args_list[-1][0][0].startswith(
            "Dynamically selected 1 default locations"
        )
    )
