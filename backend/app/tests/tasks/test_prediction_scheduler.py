...
class TestPredictionScheduler(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.scheduler = PredictionScheduler(
            ...
            config={"prediction_scheduler": {"prediction_interval_minutes": 15}},
        )
        # Setup AsyncMock for analytics_service methods that will be called by new tests
        self.mock_analytics_service.record_prediction_log = AsyncMock(
            return_value=str(random.randint(1000, 2000))
        ...
        self.original_hardcoded_locations_with_names = [
            LocationModel(
                latitude=34.0522, longitude=-118.2437, name="Los Angeles Downtown"
            ),
            LocationModel(
                latitude=40.7128, longitude=-74.0060, name="New York Times Square"
            ),
            LocationModel(
                latitude=41.8781, longitude=-87.6298, name="Chicago The Loop"
            ),
            LocationModel(
                latitude=37.7749, longitude=-122.4194, name="San Francisco Embarcadero"
            ),
            LocationModel(
                latitude=33.7490, longitude=-84.3880, name="Atlanta Centennial Park"
            ),
        ]
    ...
    async def test_load_monitored_locations_uses_priority_when_set(
        self, mock_randint, mock_random_sample
    ):
        priority_locs = [
        ...
        self.assertEqual(
            self.scheduler._priority_locations, []
        )  # Should be cleared after use
        self.scheduler.logger.info.assert_any_call(
            f"Using {len(priority_locs)} priority locations for prediction: {['Priority Spot A', 'Priority Spot B']}"
        )
        mock_randint.assert_not_called()  # Random selection should be skipped
        mock_random_sample.assert_not_called()

    @patch("app.tasks.prediction_scheduler.PredictionScheduler._get_all_configured_locations")
    @patch("app.tasks.prediction_scheduler.random.choices")
    @patch("app.tasks.prediction_scheduler.random.randint")
    async def test_load_monitored_locations_uses_default_when_no_priority(
        self, mock_randint, mock_random_choices, mock_get_all_locations
    ):
        ...
        mock_get_all_locations.return_value = self.original_hardcoded_locations_with_names
        mock_random_choices.return_value = sample_subset
        ...
        mock_randint.assert_called_once_with(
            1, max(1, int(len(self.original_hardcoded_locations_with_names) * 0.5))
        )
        # The ANY here is because the list is constructed inside _load_monitored_locations
        mock_random_choices.assert_called_once()
        call_args, call_kwargs = mock_random_choices.call_args
        self.assertEqual(call_args[0], self.original_hardcoded_locations_with_names)
        self.assertEqual(len(call_kwargs['weights']), 5)
        self.assertEqual(call_kwargs['k'], 2)
        ...
        for item in call_args[0]:
            self.assertIsInstance(item, LocationModel)
            self.assertIsNotNone(
                item.name
            )  # Check that names are present in the default list

        self.scheduler.logger.info.assert_any_call(
            "No priority locations set. Attempting to select locations based on prediction accuracy."
        )
        self.scheduler.logger.info.assert_any_call(
            f"Dynamically selected 2 locations based on accuracy weights: {[f'{loc.name if loc.name else self.scheduler._get_location_key(loc)} (W:0.80)' for loc in sample_subset]}"
        )

    @patch(
        "app.tasks.prediction_scheduler.logger"
    )  # Patch original module logger if self.scheduler.logger isn't used by determine_autonomous_actions
    def test_determine_autonomous_actions(self, mock_module_logger):
        sample_prediction = {"likelihood_score_percent": 85}
        sample_location = LocationModel(latitude=10.0, longitude=20.0, name="Test Location")

        action = self.scheduler.determine_autonomous_actions(
            sample_prediction, sample_location
        )
        ...
        mock_module_logger.info.assert_called_once_with(
            f"Determined autonomous action: {action}"
        )

    @patch.object(PredictionScheduler, "determine_autonomous_actions")
    @patch(
        "app.tasks.prediction_scheduler.logger"
    )  # logger used in _predict_and_notify
    async def test_predict_and_notify_high_likelihood(
        self, mock_internal_logger, mock_determine_actions
    ):
        sample_location = LocationModel(latitude=34.0522, longitude=-118.2437, name="Test Location")
        mock_prediction_result = {
            "likelihood_score_percent": 90,
            "recommendations": ["Slow down"],
            "message": "High likelihood of incident.",
            "severity": "high",
            "suggested_actions": ["Deploy resources"],
        }
        ...
        self.mock_analytics_service._connection_manager.broadcast_to_topic = AsyncMock()
        ...
        args, kwargs = (
            self.mock_analytics_service._connection_manager.broadcast_to_topic.call_args
        )
        sent_message_model = args[0]
        self.assertIsInstance(sent_message_model, WebSocketMessage)
        self.assertEqual(
            sent_message_model.type, WebSocketMessageTypeEnum.PREDICTION_ALERT
        )

        notification_payload = sent_message_model.data
        self.assertIsInstance(notification_payload, GeneralNotification)
        self.assertIn("Traffic Prediction - Test Location", notification_payload.title)
        self.assertEqual(notification_payload.severity, "high")
        self.assertEqual(notification_payload.suggested_actions, ["Deploy resources"])


        # Check logging within _predict_and_notify
        self.assertTrue(
            any(
                f"Sent high likelihood notification for location {sample_location.latitude},{sample_location.longitude}"
                in log_call.args[0]
        ...
        log_call_args = self.mock_analytics_service.record_prediction_log.call_args[0][
            0
        ]  # Get the log_data dict
    ...
    async def test_predict_and_notify_low_likelihood(self, mock_determine_actions):
        ...
        self.mock_analytics_service._connection_manager.broadcast_to_topic.assert_not_awaited()  # Notification should not be sent
...
# --- Tests for accuracy-based location selection ---
    ...
    @patch("app.tasks.prediction_scheduler.PredictionScheduler._get_all_configured_locations")
    @patch(
        "app.tasks.prediction_scheduler.random.choices"
    )  # Patch random.choices used in _load_monitored_locations
    async def test_load_monitored_locations_adapts_to_accuracy(self, mock_random_choices, mock_get_all_locations):
        mock_get_all_locations.return_value = self.original_hardcoded_locations_with_names
        ...
        def get_summary_side_effect(
            location_latitude, location_longitude, time_since, **kwargs
        ):
            if (
                location_latitude == loc_a.latitude
                and location_longitude == loc_a.longitude
            ):
                return {
                    "accuracy_metrics": {"incident_hit_rate": 0.8},
                    "total_verified_predictions": 10,
                    "outcomes": {"incident_occurred": 8, "no_event_detected": 2},
                }
            elif (
                location_latitude == loc_b.latitude
                and location_longitude == loc_b.longitude
            ):
                return {
                    "accuracy_metrics": {"incident_hit_rate": 0.2},
                    "total_verified_predictions": 10,
                    "outcomes": {"incident_occurred": 2, "no_event_detected": 8},
                }
            elif (
                location_latitude == loc_c.latitude
                and location_longitude == loc_c.longitude
            ):
                return {
                    "accuracy_metrics": {"incident_hit_rate": 0.5},
                    "total_verified_predictions": 2,
                    "outcomes": {"incident_occurred": 1, "no_event_detected": 1},
                }  # Low data
            return {
                "accuracy_metrics": {"incident_hit_rate": 0.5},
        ...
        mock_random_choices.return_value = [
            loc_a,
            loc_b,
        ]  # Dummy return, we care about weights
        ...
        expected_calls = [
            call(
                ...
                time_since=unittest.mock.ANY,
            )
            for loc in self.original_hardcoded_locations_with_names
        ]
        self.mock_analytics_service.get_prediction_outcome_summary.assert_has_calls(
            expected_calls, any_order=True
        )
        self.assertEqual(
            self.mock_analytics_service.get_prediction_outcome_summary.call_count,
            len(self.original_hardcoded_locations_with_names),
        )

        # Assert that the cache is populated
        self.assertIn(
            self.scheduler._get_location_key(loc_a), self.scheduler._location_accuracy_cache
        )
        self.assertEqual(
            self.scheduler._location_accuracy_cache[
                self.scheduler._get_location_key(loc_a)
            ]["accuracy_metrics"]["incident_hit_rate"],
            0.8,
        )
        ...
        self.assertEqual(
            len(passed_weights), len(self.original_hardcoded_locations_with_names)
        )
    ...
    @patch("app.tasks.prediction_scheduler.PredictionScheduler._get_all_configured_locations")
    async def test_load_monitored_locations_uses_accuracy_cache(self, mock_get_all_locations):
        mock_get_all_locations.return_value = self.original_hardcoded_locations_with_names
        await self.scheduler.set_priority_locations([])
        self.mock_analytics_service.get_prediction_outcome_summary.side_effect = (
            lambda **kwargs: {
                "accuracy_metrics": {"incident_hit_rate": 0.6},
                "total_verified_predictions": 10,
            }
        )
        ...
        self.assertIsNotNone(
            self.scheduler._last_accuracy_cache_refresh
        )  # Cache timestamp should be set


    @patch("app.tasks.prediction_scheduler.PredictionScheduler._get_all_configured_locations")
    async def test_load_monitored_locations_refreshes_accuracy_cache_after_ttl(self, mock_get_all_locations):
        mock_get_all_locations.return_value = self.original_hardcoded_locations_with_names
        await self.scheduler.set_priority_locations([])
        self.scheduler._accuracy_cache_ttl = timedelta(
            milliseconds=10
        )  # Short TTL for test
        ...
        def initial_summary_side_effect(**kwargs):
            ...
            return {
                "accuracy_metrics": {"incident_hit_rate": 0.7},
                "total_verified_predictions": 20,
            }

        self.mock_analytics_service.get_prediction_outcome_summary.side_effect = (
            initial_summary_side_effect
        )
        ...
        def refreshed_summary_side_effect(**kwargs):
            ...
            return {
                "accuracy_metrics": {"incident_hit_rate": 0.75},
                "total_verified_predictions": 22,
            }  # Slightly different data

        self.mock_analytics_service.get_prediction_outcome_summary.side_effect = (
            refreshed_summary_side_effect
        )
        ...
        self.assertTrue(
            refreshed_summary_call_count > 0, "Cache should have been refreshed"
        )
        # Verify the cache has new data (optional, main point is call count)
        loc_key = self.scheduler._get_location_key(
            self.original_hardcoded_locations_with_names[0]
        )
        if self.scheduler._location_accuracy_cache.get(loc_key):  # Ensure key exists
            self.assertAlmostEqual(
                self.scheduler._location_accuracy_cache[loc_key]["accuracy_metrics"][
                    "incident_hit_rate"
                ],
                0.75,
            )
...
# Simplified run loop test
    @patch("app.tasks.prediction_scheduler.PredictionScheduler._get_all_configured_locations")
    async def test_run_loop_uses_priority_then_default(self, mock_get_all_locations):
        mock_get_all_locations.return_value = self.original_hardcoded_locations_with_names
        self.scheduler.logger = (
            MagicMock()
        )  # Re-mock logger for this specific test if needed for call count isolation
        self.scheduler._predict_and_notify = AsyncMock()
        self.mock_analytics_service.predict_incident_likelihood = AsyncMock(return_value={"likelihood_score_percent": 50})
        ...
        self.assertEqual(self.scheduler.monitored_locations, priority_locs)
        self.assertTrue(
            self.scheduler.logger.info.call_args_list[-1][0][0].startswith(
                "Using 1 priority locations"
            )
        )
