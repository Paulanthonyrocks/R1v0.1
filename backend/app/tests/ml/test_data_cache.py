...
class TestTrafficDataCache(unittest.TestCase):
    def setUp(self):
        self.cache = TrafficDataCache(
            max_history_hours=1
        )  # Short history for easier testing
    ...
    def test_get_all_location_summaries_single_location_single_point(self):
        ...
        self.assertEqual(
            summary1["name"], f"Node at ({round(lat1, 4):.4f}, {round(lon1, 4):.4f})"
        )
    ...
    def test_get_all_location_summaries_multiple_locations_multiple_points(self):
        ...
        self.cache.add_data_point(
            lat1, lon1, ts1_new, data1_new
        ...
        summary_loc1 = next(
            s for s in summaries if s["id"] == f"{round(lat1, 4)},{round(lon1, 4)}"
        )
        summary_loc2 = next(
            s for s in summaries if s["id"] == f"{round(lat2, 4)},{round(lon2, 4)}"
        )
        ...
        self.assertEqual(
            summary_loc1["congestion_score"], data1_new["congestion_score"]
        )
        self.assertEqual(
            summary_loc1["custom_field"], data1_new["custom_field"]
        )  # Check extra fields
    ...
    def test_get_all_location_summaries_data_cleaned_if_too_old(self):
        ...
        self.assertEqual(
            len(summaries),
            0,
            "Expected no summaries as the only data point was too old and should be cleaned.",
        )
