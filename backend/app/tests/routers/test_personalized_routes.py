...
def client_fixture(mock_personalized_routing_service_fixture: MagicMock):
    ...
    app.dependency_overrides[get_personalized_routing_service] = (
        lambda: mock_personalized_routing_service_fixture
    )
...
def test_record_suggestion_feedback_success(
    client_fixture: TestClient, mock_personalized_routing_service_fixture: MagicMock
):
...
def test_record_suggestion_feedback_suggestion_not_found(
    client_fixture: TestClient, mock_personalized_routing_service_fixture: MagicMock
):
    mock_personalized_routing_service_fixture.record_suggestion_feedback.return_value = False  # Simulate service returning False
    payload = {"suggestion_id": "sugg_not_found_456", "interaction_status": "ignored"}
    response = client_fixture.post(FEEDBACK_ENDPOINT_URL, json=payload)
    ...
    assert (
        ...
        == "Suggestion ID not found, user mismatch, or invalid data. Feedback not recorded."
    )
    mock_personalized_routing_service_fixture.record_suggestion_feedback.assert_called_once()


def test_record_suggestion_feedback_validation_error_rating_out_of_bounds(
    ...
    assert (
        response_low.status_code == 422
    )  # Unprocessable Entity for Pydantic validation errors
    assert (
        "rating" in response_low.text
    )  # Check that the error message mentions 'rating'
...
def test_record_suggestion_feedback_unauthenticated(
    client_fixture: TestClient, mock_personalized_routing_service_fixture: MagicMock
):
    ...
    )  # Or whatever detail your auth setup returns
...
def test_record_suggestion_feedback_service_exception(
    client_fixture: TestClient, mock_personalized_routing_service_fixture: MagicMock
):
    # Simulate an unexpected exception in the service layer
    mock_personalized_routing_service_fixture.record_suggestion_feedback.side_effect = (
    ...
    )  # As per the endpoint's general exception handler
    assert (
        "An unexpected error occurred while recording feedback"
        in response.json()["detail"]
    )
    assert (
        "Unexpected service error" in response.json()["detail"]
    )  # Check if original error message is included
...
def test_record_suggestion_feedback_minimal_payload(
    client_fixture: TestClient, mock_personalized_routing_service_fixture: MagicMock
):
    mock_personalized_routing_service_fixture.record_suggestion_feedback.return_value = True
    payload = {
