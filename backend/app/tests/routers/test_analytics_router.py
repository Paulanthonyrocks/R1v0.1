...
class TestAnalyticsRouterNodesCongestion(unittest.TestCase):
    ...
    def test_get_nodes_congestion_success(self):
        ...
        mock_analytics_service_instance.get_all_location_congestion_data = AsyncMock(
            return_value=mock_node_data
        )
        ...
        self.assertEqual(
            response_json["data"]["nodes"][0]["id"], mock_node_data[0]["id"]
        )
        self.assertEqual(
            response_json["data"]["nodes"][0]["name"], mock_node_data[0]["name"]
        )
        self.assertEqual(
            response_json["data"]["nodes"][0]["congestion_score"],
            mock_node_data[0]["congestion_score"],
        )
        ...
        self.assertEqual(
            isoparse(response_json["data"]["nodes"][0]["timestamp"]).replace(
                tzinfo=None
            ),
            isoparse(mock_node_data[0]["timestamp"]).replace(tzinfo=None),
        )
    ...
    def test_get_nodes_congestion_service_error(self):
        ...
        self.assertEqual(
            response_json["detail"], "Failed to retrieve node congestion data."
        )
        mock_analytics_service_instance.get_all_location_congestion_data.assert_awaited_once()
