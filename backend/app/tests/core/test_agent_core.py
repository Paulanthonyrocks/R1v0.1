...
class TestAgentCore(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_personalized_routing_service = MagicMock(
            spec=PersonalizedRoutingService
        )
        ...
        self.mock_prediction_scheduler._predict_and_notify = AsyncMock(
            return_value=None
        )
        self.mock_prediction_scheduler.set_priority_locations = (
            AsyncMock()
        )  # Make set_priority_locations an AsyncMock

        self.mock_personalized_routing_service.proactively_suggest_route = AsyncMock(
            return_value="Sample suggestion"
        )

        # Configure AnalyticsService mocks
        self.mock_kpi_summary_default = {
            "overall_congestion_level": "LOW",
            "active_monitored_locations": 2,
            "average_speed_kmh": 60,
            "total_vehicle_flow_estimate": 1000,
        }
        self.mock_alert_summary_default = {
            "critical_unack_alert_count": 0,
            "recent_critical_types": [],
        }

        self.mock_analytics_service.get_current_system_kpis_summary.return_value = (
            self.mock_kpi_summary_default
        )
        self.mock_analytics_service.get_critical_alert_summary = AsyncMock(
            return_value=self.mock_alert_summary_default
        )
        ...
        self.mock_analytics_service.predict_incident_likelihood = (
            AsyncMock()
        )  # Added for new predictive logic

        # Mock for PersonalizedRoutingService method used in user-specific alerts
        self.mock_personalized_routing_service.get_user_common_travel_patterns = (
            AsyncMock(return_value=[])
        )  # Default to no patterns
    ...
    def test_init_stores_services(self):
        self.assertEqual(
            self.agent_core.prediction_scheduler, self.mock_prediction_scheduler
        )
        self.assertEqual(
            self.agent_core.personalized_routing_service,
            self.mock_personalized_routing_service,
        )
        self.assertEqual(
            self.agent_core.analytics_service, self.mock_analytics_service
    ...
    async def test_run_decision_cycle_happy_path(self, mock_agent_logger):
        ...
        self.mock_prediction_scheduler.monitored_locations = (
            locations_to_monitor  # Set after load is called
        )

        # Make _predict_and_notify an async mock
        self.mock_prediction_scheduler._predict_and_notify = MagicMock(
            side_effect=lambda loc: asyncio.sleep(0)
        )
        ...
        self.mock_personalized_routing_service.proactively_suggest_route.assert_awaited_once_with(
            sample_user_id
        )
        ...
        mock_agent_logger.info.assert_any_call(
            f"Proactive route suggestion for user {sample_user_id}: Sample suggestion"
        )

        mock_agent_logger.info.assert_any_call(
            f"AgentCore received System KPIs: {json.dumps(self.mock_kpi_summary_default, indent=2)}"
        )
        mock_agent_logger.info.assert_any_call(
            f"AgentCore received Critical Alert Summary: {json.dumps(self.mock_alert_summary_default, indent=2)}"
        )
        ...
        self.assertTrue(
            any(
                expected_summary_log_part in call_arg[0][0]
        ...
        mock_agent_logger.info.assert_any_call(
            "AgentCore action: System status within acceptable parameters, no new global operational alert issued by AgentCore."
        )

        mock_agent_logger.info.assert_any_call("AgentCore decision cycle completed.")

    @patch("app.core.agent_core.logger")
    async def test_run_decision_cycle_triggers_high_congestion_alert(
        self, mock_agent_logger
    ):
        # Scenario: High congestion, no critical alerts
        high_congestion_kpis = {
            "overall_congestion_level": "HIGH",
            "average_speed_kmh": 30,
            "total_vehicle_flow_estimate": 2000,
        ...
        self.mock_analytics_service.get_critical_alert_summary.return_value = (
            no_critical_alerts
        )
        ...
        self.assertTrue(
            any(
                "AgentCore action: Issued OPERATIONAL ALERT. Title: 'High System Congestion', Severity: error"
                in log_call.args[0]
    ...
    @patch("app.core.agent_core.logger")
    async def test_run_decision_cycle_triggers_critical_alerts_alert(
        self, mock_agent_logger
    ):
        ...
        low_congestion_kpis = {
            ...
            "average_speed_kmh": 60,
            "total_vehicle_flow_estimate": 1000,
            "active_monitored_locations": 3,
        }
        many_critical_alerts = {
            "critical_unack_alert_count": 3,
            "recent_critical_types": ["typeA", "typeB"],
        }
        self.mock_analytics_service.get_current_system_kpis_summary.return_value = (
            low_congestion_kpis
        )
        ...
        if (
            low_congestion_kpis["overall_congestion_level"] != "UNKNOWN"
        ):  # Check if it might combine with a low congestion message
        ...
        self.assertTrue(
            any(
                f"AgentCore action: Issued OPERATIONAL ALERT. Title: '{expected_title}', Severity: {expected_severity}"
                in log_call.args[0]
                for log_call in mock_agent_logger.info.call_args_list
            )
        )
    ...
    @patch("app.core.agent_core.logger")
    async def test_run_decision_cycle_triggers_high_congestion_and_critical_alerts(
        self, mock_agent_logger
    ):
        ...
        high_congestion_kpis = {
            ...
            "total_vehicle_flow_estimate": 2500,
            "active_monitored_locations": 6,
        }
        some_critical_alerts = {
            "critical_unack_alert_count": 1,
            "recent_critical_types": ["typeC"],
        }  # CRITICAL_ALERT_COUNT_THRESHOLD_FOR_HIGH_CONGESTION is 0
        self.mock_analytics_service.get_current_system_kpis_summary.return_value = (
            high_congestion_kpis
        )
        self.mock_analytics_service.get_critical_alert_summary.return_value = (
        ...
        self.assertTrue(
            any(
                "AgentCore action: Issued OPERATIONAL ALERT. Title: 'High System Load & Critical Alerts', Severity: error"
                in log_call.args[0]
                for log_call in mock_agent_logger.info.call_args_list
            )
        )
    ...
    async def test_run_decision_cycle_no_locations(self, mock_agent_logger):
        ...
        mock_agent_logger.warning.assert_any_call(
            "No monitored locations loaded by PredictionScheduler. Skipping prediction notifications."
        )
    ...
    @patch("app.core.agent_core.logger")
    async def test_run_decision_cycle_routing_suggestion_is_none(
        self, mock_agent_logger
    ):
        ...
        self.mock_personalized_routing_service.proactively_suggest_route = MagicMock(
            return_value=asyncio.Future()
        )
        self.mock_personalized_routing_service.proactively_suggest_route.return_value.set_result(
            None
        )
        ...
        mock_agent_logger.info.assert_any_call(
            f"No proactive route suggestion generated for user {sample_user_id}."
        )
        # Analytics service calls should still happen
        self.mock_analytics_service.get_current_system_kpis_summary.assert_called_once()
        self.mock_analytics_service.get_critical_alert_summary.assert_awaited_once()  # Now async
...
def async_test(f):
    ...
    @async_test
    @patch(
        "app.core.agent_core.datetime"
    )  # Mock datetime within the agent_core module where it's used by the method
    async def test_determine_next_travel_prediction_time_logic(
        self, mock_core_datetime_module
    ):  # mock_core_datetime_module is the mocked datetime in agent_core
        ...
        pattern_mw = CommonTravelPattern(
            pattern_id="p_mw",
            user_id="u1",
            start_location_summary={},
            end_location_summary={},
            time_of_day_group="morning_weekday",
            days_of_week=[0, 1, 2, 3, 4],
            frequency_score=1.0,
        )
        expected_time_mw = datetime(2023, 1, 2, 8, 0, 0)  # Next day (Monday) at 8 AM
        result_mw = await self.agent_core._determine_next_travel_prediction_time(
            pattern_mw, current_time_sunday_eve
        )
        ...
        pattern_ew = CommonTravelPattern(
            ...
            start_location_summary={},
            end_location_summary={},
            time_of_day_group="evening_weekday",
            days_of_week=[0, 1, 2, 3, 4],
            frequency_score=1.0,
        )
        expected_time_ew = datetime(2023, 1, 2, 17, 0, 0)  # Same day (Monday) at 5 PM
        result_ew = await self.agent_core._determine_next_travel_prediction_time(
            pattern_ew, current_time_monday_morn
        )
        ...
        result_tue_morn = await self.agent_core._determine_next_travel_prediction_time(
            pattern_mw, current_time_monday_midday
        )
        ...
        pattern_wea = CommonTravelPattern(
            ...
            start_location_summary={},
            end_location_summary={},
            time_of_day_group="afternoon_weekend",
            days_of_week=[5, 6],
            frequency_score=1.0,
        )
        expected_time_sat_aft = datetime(
            2023, 1, 7, 15, 0, 0
        )  # Saturday 3 PM (target_hour for afternoon is 15)
        result_sat_aft = await self.agent_core._determine_next_travel_prediction_time(
            pattern_wea, current_time_friday_morn
        )
        self.assertEqual(result_sat_aft, expected_time_sat_aft)

        # Scenario 5: Target prediction time is less than 1 hour in the future
        current_time_just_before = datetime(
            2023, 1, 2, 7, 30, 0
        )  # Monday 7:30 AM, pattern for Mon 8 AM
        # Expected: Tuesday 8 AM, because Mon 8 AM is not > (Mon 7:30 AM + 1 hr)
        expected_time_skip_today = datetime(2023, 1, 3, 8, 0, 0)
        result_skip_today = (
            await self.agent_core._determine_next_travel_prediction_time(
                pattern_mw, current_time_just_before
            )
        )
        ...
        result_no_match = await self.agent_core._determine_next_travel_prediction_time(
            pattern_no_match_days, current_time_sunday_eve
        )
        ...
        result_next_week_morn = (
            await self.agent_core._determine_next_travel_prediction_time(
                pattern_mw, current_time_monday_eve
            )
        )
    ...
    @async_test
    @patch("app.core.agent_core.logger")
    async def test_run_decision_cycle_operational_alert_with_specific_suggested_actions(
        self, mock_agent_logger
    ):
    ...
    @async_test
    @patch("app.core.agent_core.logger")
    async def test_run_decision_cycle_predictive_user_alert_high_likelihood(
        self, mock_agent_logger
    ):
        ...
        self.mock_personalized_routing_service.get_user_common_travel_patterns.return_value = [
            test_pattern
        ]
        ...
        fixed_future_time = datetime.now() + timedelta(
            hours=3
        )  # Ensure this is truly in the future for the test
        # If AgentCore.datetime.now() is patched, ensure fixed_future_time is relative to that mocked 'now'

        with patch.object(
            self.agent_core,
            "_determine_next_travel_prediction_time",
            new_callable=AsyncMock,
        ) as mock_determine_time:
            mock_determine_time.return_value = fixed_future_time

            # Mock predict_incident_likelihood to return high likelihood
            high_likelihood_prediction = {
                "likelihood_score_percent": 75,
                "details": "High chance of delays",
            }
            self.mock_analytics_service.predict_incident_likelihood.return_value = (
                high_likelihood_prediction
            )

            await self.agent_core.run_decision_cycle(sample_user_id=sample_user_id)

            mock_determine_time.assert_awaited_once_with(
                test_pattern, ANY
            )  # ANY for current_time
            ...
            call_args_pred = (
                self.mock_analytics_service.predict_incident_likelihood.call_args[1]
            )  # kwargs
            self.assertIsInstance(call_args_pred["location"], LocationModel)
            self.assertEqual(
                call_args_pred["location"].latitude, mock_loc_summary_end["latitude"]
            )
            self.assertEqual(
                call_args_pred["location"].longitude, mock_loc_summary_end["longitude"]
            )
            ...
            args, kwargs = (
                self.mock_analytics_service.send_user_specific_alert.call_args
            )
    ...
    @async_test
    @patch("app.core.agent_core.logger")
    async def test_run_decision_cycle_no_predictive_user_alert_low_likelihood(
        self, mock_agent_logger
    ):  # Renamed
        ...
        common_patterns = [
            CommonTravelPattern(
                pattern_id="p1",
                user_id=sample_user_id,
                start_location_summary={"name": "H"},
                end_location_summary={"name": "W"},
                time_of_day_group="morning_weekday",
                days_of_week=[0, 1, 2, 3, 4],
                frequency_score=5.0,
            )
        ...
        with patch(
            "app.core.agent_core.datetime"
        ) as mock_datetime:  # Match time for pattern relevance
    ...
    @async_test
    @patch("app.core.agent_core.logger")
    async def test_run_decision_cycle_no_user_specific_alert_if_no_common_patterns(
        self, mock_agent_logger
    ):
