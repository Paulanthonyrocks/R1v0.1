...
class UserPreferenceLearner:
    ...
    def _identify_common_destinations(
        self, history: List[RouteHistoryEntry]
    ) -> List[Dict[str, Any]]:
        ...
        for label in unique_labels:
            ...
            cluster_entries = [
                entry for entry, is_in_cluster in zip(history, mask) if is_in_cluster
            ]
            ...
            common_destinations.append(
                {
                    ...
                    "average_duration": np.mean(
                        [entry.duration_minutes for entry in cluster_entries]
                    ),
                }
            )
    ...
    def _extract_time_patterns(
        ...
        for route_key in patterns:
            ...
            patterns[route_key] = {
                tod: count / total for tod, count in patterns[route_key].items()
            }
    ...
    def _is_similar_location(
        self, loc1: Dict[str, float], loc2: Dict[str, float], threshold: float = 0.01
    ) -> bool:
        """Check if two locations are similar within a threshold"""
        return (
            abs(loc1["latitude"] - loc2["latitude"]) < threshold
            and abs(loc1["longitude"] - loc2["longitude"]) < threshold
        )
