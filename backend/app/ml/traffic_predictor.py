...
class TrafficPredictor:
    ...
    def _initialize_model(self):
        ...
        try:
            ...
            self.model.add(tf.keras.layers.LSTM(64, return_sequences=True, input_shape=(self.sequence_length, 14)))
    ...
    def prepare_features(self, traffic_data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Time-based features
        traffic_data['hour_sin'] = np.sin(2 * np.pi * traffic_data['timestamp'].dt.hour / 24)
        traffic_data['hour_cos'] = np.cos(2 * np.pi * traffic_data['timestamp'].dt.hour / 24)
        traffic_data['day_of_week_sin'] = np.sin(2 * np.pi * traffic_data['timestamp'].dt.dayofweek / 7)
        traffic_data['day_of_week_cos'] = np.cos(2 * np.pi * traffic_data['timestamp'].dt.dayofweek / 7)
        traffic_data['is_weekend'] = traffic_data['is_weekend'].astype(int)

        # One-hot encode road_type
        traffic_data = pd.get_dummies(traffic_data, columns=['road_type'], prefix='road_type')
        ...
        features = traffic_data[[
            ...
            'weather_conditions_temperature', 'weather_conditions_precipitation', 'truck_percentage',
            'road_type_major_artery', 'road_type_highway', 'road_type_local_road'
        ]]
        return features
    ...
    def predict_incident_likelihood(self, 
        ...
        if not recent_traffic_data:
            ...
            for col in ['vehicle_count', 'average_speed', 'congestion_level', 'congestion_score',
                         'processing_timestamp', 'status', 'hour_of_day', 'day_of_week', 'is_weekend',
                         'road_type', 'weather_conditions_temperature', 'weather_conditions_precipitation',
                         'truck_percentage', 'is_outlier', 'incident_occurred']:
                if col not in input_features.columns:
                    if col in ['vehicle_count', 'incident_occurred']:
                        input_features[col] = 0
                    elif col in ['average_speed', 'congestion_level', 'congestion_score', 'truck_percentage',
                        ...
                        input_features[col] = (input_features['timestamp'].dt.dayofweek >= 5).astype(int)
                    elif col == 'road_type':
                        input_features[col] = 'major_artery' # Default road type

        if self.model is None:
            logger.warning("Model not loaded or trained. Returning rule-based prediction.")
            # Fallback to rule-based if model is not available
            return self._rule_based_prediction(location, prediction_time, input_features)
        ...
        try:
            ...
            scaled_features_reshaped = np.tile(scaled_features, (1, self.sequence_length, 1))

            prediction = self.model.predict(scaled_features_reshaped)[0][0] # Get scalar prediction
            
            incident_likelihood = float(prediction)
            confidence_score = 0.8 # Placeholder, could be derived from model output or ensemble
            ...
            return {
                ...
                "contributing_factors": self._get_contributing_factors(input_features.iloc[0], incident_likelihood),
                "recommendations": self._generate_recommendations(incident_likelihood, self._get_contributing_factors(input_features.iloc[0], incident_likelihood))
            }
    ...
    def _rule_based_prediction(self, location: Dict[str, Any], prediction_time: datetime, input_features: pd.DataFrame) -> Dict[str, Any]:
        ...
        avg_speed = input_features['average_speed'].iloc[0] if not input_features.empty and 'average_speed' in input_features.columns else 0.0
        vehicle_count = input_features['vehicle_count'].iloc[0] if not input_features.empty and 'vehicle_count' in input_features.columns else 0
    ...
    def _get_contributing_factors(self, data_row: pd.Series, likelihood: float) -> List[str]:
        factors = []
        if likelihood > 0.6:
    ...
    def _create_sequences(self, features: np.ndarray, target: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        X, y = [], []
        for i in range(len(features) - self.sequence_length + 1): # Adjusted loop for sequence creation
            X.append(features[i : i + self.sequence_length])
            y.append(target[i + self.sequence_length - 1]) # Target corresponds to the last element in the sequence
