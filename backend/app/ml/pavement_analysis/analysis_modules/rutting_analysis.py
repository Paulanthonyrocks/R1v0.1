...
def analyze_rutting_bbox(
    ...
    if w <= 0 or h <= 0:
        logger.warning(
            f"analyze_rutting_bbox received invalid box dimensions: w={w}, h={h}"
        )
        return {}
    ...
    try:
        ...
        measurements["bbox_height_mm"] = round(
            h * px_to_mm_y, 2
        )  # This is length along the image axis
        measurements["area_sq_m_bbox"] = round(
            (w * px_to_mm_x * h * px_to_mm_y) / (1000 * 1000), 4
        )
        ...
        logger.debug(
            f"Rutting analysis (bbox based) for box {detection_box}: {measurements}"
        )

    except Exception as e:
        logger.error(
            f"Error analyzing rutting in ROI {detection_box}: {e}", exc_info=True
        )
