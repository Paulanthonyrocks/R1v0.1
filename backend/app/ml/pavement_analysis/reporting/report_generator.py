...
def generate_analysis_report(
    ...
    try:
        ...
        for distress in distresses:
            ...
            current_count = report_data["summary"]["distress_types_count"].get(
                d_type, 0
            )
        ...
        if include_visualizations:
            ...
            if image is not None:
                ...
                for distress in distresses:
                    ...
                    d_type_str = distress.get(
                        "type", DistressType.UNKNOWN_DISTRESS.value
                    )
                    if not isinstance(d_type_str, str):
                        d_type_str = d_type_str.value  # ensure string key

                    color = DISTRESS_COLORS.get(
                        d_type_str, DISTRESS_COLORS[DistressType.UNKNOWN_DISTRESS.value]
                    )
                    ...
                    if bbox and all(k in bbox for k in ("x1", "y1", "x2", "y2")):
                        ...
                        if (
                            ...
                            and distress["confidence"] is not None
                        ):
                        ...
                        if "estimated_diameter_mm" in measurements:
                            label_parts.append(
                                f"D:{measurements['estimated_diameter_mm']:.1f}mm"
                            )
                        ...
                        text_y_pos = max(
                            label_height + baseline // 2, y1 - baseline // 2 - 2
                        )
                        ...
                        logger.warning(
                            f"Skipping drawing for distress due to missing/invalid bbox: {distress.get('type')}"
                        )

                # Add PCI score
                pci_text = f"PCI: {pci_score:.1f}"
                (text_width, text_height), baseline = cv2.getTextSize(
                    pci_text, cv2.FONT_HERSHEY_SIMPLEX, 1, 2
                )
                cv2.rectangle(
                    vis_image,
                    (5, 5),
                ...
                if distress_types_counts:
                    plt.bar(
                        distress_types_counts.keys(),
                        distress_types_counts.values(),
                        color=[
                            DISTRESS_COLORS.get(k, (0.5, 0.5, 0.5))
                            for k in distress_types_counts.keys()
                        ],
                    )
                    ...
                    logger.info(
                        "Skipping distress distribution plot as no distresses were found or counted."
                    )
            else:
                logger.warning(
                    f"Could not read image at {image_path} for visualization."
                )
        ...
        logger.error(
            f"Error generating analysis report for {base_filename}: {e}", exc_info=True
        )
