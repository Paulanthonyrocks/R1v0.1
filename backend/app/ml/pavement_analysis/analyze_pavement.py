...
async def analyze_pavement_image(
    image_data: bytes, image_filename: str = "uploaded_image.png"
) -> PavementAnalysisResponse:
    """
    ...
    try:
        ...
        logger.info(
            f"Starting pavement analysis for image: {image_filename} (Timestamp: {timestamp})"
        )
        ...
        if (
            calibration_params
            and "camera_matrix" in calibration_params
            and "dist_coeffs" in calibration_params
        ):
        ...
        processed_image_for_ml, _ = await _run_in_executor(
            preprocess_image, undistorted_image.copy()
        )
        ...
        ml_detections_raw = await _run_in_executor(
            detect_distresses_ml, undistorted_image, ml_model
        )
        ...
        for det_idx, raw_det in enumerate(ml_detections_raw):
            try:
                ...
                if bbox is None:
                    logger.warning(
                        f"Skipping detection {det_idx} due to missing bounding box."
                    )
                    continue
                ...
                if any(v is None for v in [x1, y1, x2, y2]):
                    logger.warning(
                        f"Skipping detection {det_idx} due to incomplete bounding box values."
                    )
                    continue
                ...
                if "crack" in distress_type_str:
                    if segmentation_mask is not None and segmentation_mask.size > 0:
                        ...
                        if contours:
                            ...
                            measurements_data = measure_crack_contour(
                                main_contour,
                            ...
                            logger.warning(
                                f"Crack detection {det_idx} had a mask but no contours found."
                            )
                    else:
                        logger.warning(
                            f"Crack detection {det_idx} ({distress_type_str}) has no segmentation mask. Measurement will be basic."
                        )
                        ...
                        measurements_data["area_sq_m"] = (
                            rect_box[2] * px_to_mm_x * rect_box[3] * px_to_mm_y
                        ) / (1000 * 1000)
                    ...
                    if segmentation_mask is not None and segmentation_mask.size > 0:
                        ...
                        if contours:
                            main_contour = max(contours, key=cv2.contourArea)
                            measurements_data = measure_pothole_contour(
                                main_contour,
                                undistorted_image.shape[:2],
                                calibration_params,
                            )
                            ...
                            logger.warning(
                                f"Pothole detection {det_idx} had a mask but no contours found."
                            )
                    else:
                        logger.warning(
                            f"Pothole detection {det_idx} has no segmentation mask. Measurement will be basic."
                        )
                        px_to_mm_x, px_to_mm_y = get_pixel_to_mm_ratio(
                            calibration_params,
                            image_height=img_height,
                        ...
                        measurements_data["estimated_diameter_mm"] = (
                            (rect_box[2] * px_to_mm_x) + (rect_box[3] * px_to_mm_y)
                        ) / 2
                        measurements_data["area_sq_m"] = (
                            rect_box[2] * px_to_mm_x * rect_box[3] * px_to_mm_y
                    ...
                    measurements_data = analyze_rutting_bbox(
                        undistorted_image, rect_box, calibration_params
                    )
                else:
                    logger.warning(
                        f"No specific measurement module for distress type: {distress_type_str}. Basic BBox metrics."
                    )
                    px_to_mm_x, px_to_mm_y = get_pixel_to_mm_ratio(
                        calibration_params,
                        image_height=img_height,
                ...
                if (
                    "area_sq_m" not in measurements_data
                    and "area_sq_m_bbox" in measurements_data
                ):
                    measurements_data["area_sq_m"] = measurements_data["area_sq_m_bbox"]
                elif (
                    "area_sq_m" not in measurements_data
                    and "width_mm" in measurements_data
                    and "length_mm" in measurements_data
                ):
                    measurements_data["area_sq_m"] = (
                        measurements_data["width_mm"] * measurements_data["length_mm"]
                    ) / (1000 * 1000)
                ...
                try:
                    ...
                    logger.warning(
                        f"Unknown distress type string '{distress_type_str}'. Defaulting to UNKNOWN_DISTRESS."
                    )
                ...
                logger.error(
                    f"Error processing or measuring detection {det_idx} ({raw_det.get('type')}): {e_meas}",
                    exc_info=True,
                )
        ...
        pci_score = await _run_in_executor(calculate_pci, distress_data_for_pci)
        logger.info(f"Calculated PCI score: {pci_score:.2f}")

        # 6. Save analyzed image (with detections drawn)
        final_image_for_report_path = (
            image_output_dir / f"{base_filename}_base_image.png"
        )
        ...
        # Construct response
        static_image_path_prefix = (
            Path("/static") / Path(config["image_output_dir"]).name
        )
        static_report_path_prefix = (
            Path("/static") / Path(config["report_output_dir"]).name
        )

        vis_image_url = None
        if report_files_dict.get("visualization"):
            vis_image_url = str(
                static_image_path_prefix / Path(report_files_dict["visualization"]).name
            )

        report_json_url = None
        if report_files_dict.get("report"):
            report_json_url = str(
                static_report_path_prefix / Path(report_files_dict["report"]).name
            )
        ...
        if report_files_dict.get("plot"):
            analysis_response.plot_url = str(
                static_report_path_prefix / Path(report_files_dict["plot"]).name
            )

        logger.info(
            f"Successfully completed pavement analysis for image: {image_filename}"
        )
        ...
        logger.error(
            f"Unexpected error analyzing pavement image {image_filename}: {e}",
            exc_info=True,
        )
