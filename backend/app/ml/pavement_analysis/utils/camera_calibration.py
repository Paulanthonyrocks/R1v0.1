...
def get_pixel_to_mm_ratio(
    ...
    try:
        ...
        if (
            calibration_params
            and "pixel_to_mm_ratio_x" in calibration_params
            and "pixel_to_mm_ratio_y" in calibration_params
        ):
            return float(calibration_params["pixel_to_mm_ratio_x"]), float(
                calibration_params["pixel_to_mm_ratio_y"]
            )
        ...
        # Calculate from camera parameters if distance is provided (simplified)
            ...
            calibration_params
            and distance_to_surface is not None
            ...
            logger.warning(
                "Pixel to mm ratio from focal length and distance is experimental. Consider using direct calibration."
            )
        ...
        logger.warning(
            "Using default pixel to mm ratios. This should be calibrated for accuracy."
        )
        return (
            DEFAULT_PIXEL_TO_MM_X * scale_factor,
            DEFAULT_PIXEL_TO_MM_Y * scale_factor,
        )
