...
from app.services.personalized_routing_service import (
    ...
    CommonTravelPattern,
)  # Ensure CommonTravelPattern is directly importable or accessed via service
...
class AgentCore:
    def __init__(
        ...
        self.logger = logger
        logger.info(
            "AgentCore initialized with PredictionScheduler, PersonalizedRoutingService, and AnalyticsService."
        )

    async def _determine_next_travel_prediction_time(
        self, pattern: CommonTravelPattern, current_dt: datetime
    ) -> Optional[datetime]:
        """
        ...
        """
        self.logger.debug(
            f"Determining next travel time for pattern {pattern.pattern_id} (Time: {pattern.time_of_day_group}, Days: {pattern.days_of_week}) from current_dt: {current_dt}"
        )
        ...
        if "morning" in time_group:
            ...
            self.logger.warning(
                f"Unknown time_of_day_group '{pattern.time_of_day_group}' for pattern {pattern.pattern_id}. Cannot determine target hour."
            )
            return None
        ...
        for i in range(8):  # Check today and next 7 days
            ...
            if next_date_to_check.weekday() in pattern.days_of_week:
                ...
                if potential_prediction_dt > current_dt + timedelta(hours=1):
                    self.logger.info(
                        f"Determined next prediction time for pattern {pattern.pattern_id}: {potential_prediction_dt}"
                    )
                    return potential_prediction_dt

        self.logger.info(
            f"No suitable future prediction time found within 7 days for pattern {pattern.pattern_id}."
        )
        return None
    ...
    async def run_decision_cycle(self, sample_user_id: str = "user_agent_test_123"):
        ...
        system_kpis: Dict[str, Any] = (
            self.analytics_service.get_current_system_kpis_summary()
        )
        self.logger.info(
            f"AgentCore received System KPIs: {json.dumps(system_kpis, indent=2)}"
        )

        self.logger.info(
            "Fetching critical alert summary for AgentCore decision making..."
        )
        alert_summary: Dict[
            str, Any
        ] = await self.analytics_service.get_critical_alert_summary()
        self.logger.info(
            f"AgentCore received Critical Alert Summary: {json.dumps(alert_summary, indent=2)}"
        )
        ...
        sample_priority_locations = [
            ...
            LocationModel(
                latitude=37.7749, longitude=-122.4194, name="SF Critical Junction"
            ),  # Example
        ]
        ...
        if True:  # For now, always set sample priorities to demonstrate the mechanism
            await self.prediction_scheduler.set_priority_locations(
                sample_priority_locations
            )
            priority_location_names = [
                loc.name for loc in sample_priority_locations if loc.name
            ]
            self.logger.info(
                f"AgentCore instructed PredictionScheduler to prioritize locations: {priority_location_names if priority_location_names else 'unnamed locations'}"
            )
        else:
            ...
            self.logger.info(
                "AgentCore: Conditions do not require specific priority locations for PredictionScheduler."
            )

        # --- Personalized Routing Phase (can remain, as it's user-specific) ---
        logger.info(
            f"Attempting to generate proactive route suggestion for user: {sample_user_id}..."
        )
        try:
            suggestion = (
                await self.personalized_routing_service.proactively_suggest_route(
                    sample_user_id
                )
            )
            if suggestion:
                logger.info(
                    f"Proactive route suggestion for user {sample_user_id}: {suggestion}"
                )
            else:
                logger.info(
            ...
            logger.error(
                f"Error during proactive route suggestion for user {sample_user_id}: {e}"
            )
        ...
        CRITICAL_ALERT_COUNT_THRESHOLD_FOR_HIGH_CONGESTION = (
            0  # Alert if HIGH congestion AND >0 critical
        )
        CRITICAL_ALERT_COUNT_THRESHOLD_STANDALONE = (
            2  # Alert if >2 critical alerts, regardless of congestion
        )
        ...
        current_congestion_level = system_kpis.get(
            "overall_congestion_level", "UNKNOWN"
        )
        avg_speed = system_kpis.get(
            "average_speed_kmh", -1.0
        )  # Use -1 to indicate unknown if not present
        ...
        if current_congestion_level == "HIGH":
            if (
                avg_speed != -1 and avg_speed < 15
            ):  # Severe congestion if avg speed is very low
            ...
            trigger_operational_alert = (
                True  # Alert even for medium if other factors exist or just to inform
            )
        ...
        if (
            critical_alerts_count_val > CRITICAL_ALERT_COUNT_THRESHOLD_STANDALONE
        ):  # e.g., threshold = 2
            if (
                not trigger_operational_alert
            ):  # If congestion didn't trigger an alert, this will be the primary
                trigger_operational_alert = True
                ...
                operational_alert_severity = (
                    "error"  # Higher than warning if many criticals
                )
            ...
            suggested_actions_for_alert.append(
                "Prioritize investigation of critical alerts."
            )
            if any(
                "ACCIDENT" in str(type_).upper() for type_ in recent_critical_types
            ):  # Check if any type string contains "ACCIDENT"
                suggested_actions_for_alert.append(
                    "Verify accident reports and dispatch emergency services if needed."
                )
                suggested_actions_for_alert.append(
                    "Assess impact of any accidents on traffic flow and adjust signal timings accordingly."
                )

        # Fallback for very high number of critical alerts, even if congestion is low
        elif (
            critical_alerts_count_val
            > CRITICAL_ALERT_COUNT_THRESHOLD_FOR_HIGH_CONGESTION
            and not trigger_operational_alert
        ):  # e.g. threshold = 0
            trigger_operational_alert = True
            ...
            operational_alert_severity = "warning"
            suggested_actions_for_alert.append(
                "Review critical alerts and assess potential impact."
            )
        ...
        if trigger_operational_alert:
            ...
            await self.analytics_service.broadcast_operational_alert(
                ...
                suggested_actions=unique_suggested_actions
                if unique_suggested_actions
                else None,
            )
            self.logger.info(
                f"AgentCore action: Issued OPERATIONAL ALERT. Title: '{operational_alert_title}', Severity: {operational_alert_severity}, Actions: {unique_suggested_actions}"
            )
        else:
            self.logger.info(
        ...
        for user_id in sample_user_ids_for_proactive_alerts:
            ...
            try:
                common_patterns = await self.personalized_routing_service.get_user_common_travel_patterns(
                    user_id=user_id,
                    top_n=3,  # Get top 3 patterns
                )

                if not common_patterns:
                    self.logger.info(
                        f"No common travel patterns found for user {user_id} to make predictions."
                    )
                    continue

                for pattern in common_patterns:
                    self.logger.debug(
                        f"Evaluating pattern for user {user_id}: ID {pattern.pattern_id}, To: {pattern.end_location_summary.get('name', 'Unknown Dest')}, Time Group: {pattern.time_of_day_group}"
                    )

                    prediction_target_time = (
                        await self._determine_next_travel_prediction_time(
                    ...
                    if not prediction_target_time:
                        self.logger.debug(
                            f"No suitable future prediction time for pattern {pattern.pattern_id} for user {user_id}."
                        )
                        continue
                    ...
                    if (
                        not dest_summary
                        or not isinstance(dest_summary.get("latitude"), (float, int))
                        or not isinstance(dest_summary.get("longitude"), (float, int))
                    ):
                        self.logger.warning(
                            f"Pattern {pattern.pattern_id} for user {user_id} has invalid destination summary: {dest_summary}. Skipping prediction."
                        )
                        continue
                    ...
                    self.logger.info(
                        f"Requesting incident likelihood prediction for user {user_id}, pattern {pattern.pattern_id} (dest: {destination_location_model.name}), target time: {prediction_target_time}"
                    )
                    prediction_result = (
                        await self.analytics_service.predict_incident_likelihood(
                    ...
                    if (
                        prediction_result
                        and prediction_result.get("likelihood_score_percent", 0)
                        > PREDICTIVE_ALERT_LIKELIHOOD_THRESHOLD
                    ):
                        score = prediction_result["likelihood_score_percent"]
                        dest_name = (
                            destination_location_model.name
                            or f"area around ({destination_location_model.latitude:.2f}, {destination_location_model.longitude:.2f})"
                        )
                        time_formatted = prediction_target_time.strftime(
                            "%I:%M %p on %A, %b %d"
                        )
                        ...
                        await self.analytics_service.send_user_specific_alert(
                            user_id=user_id, notification_model=alert_payload
                        )
                        self.logger.info(
                            f"Sent predictive alert to user {user_id} for pattern {pattern.pattern_id} (destination: {dest_name}, score: {score}%)."
                        )
                    else:
                        self.logger.info(
                            f"Likelihood for pattern {pattern.pattern_id} (user {user_id}, dest: {destination_location_model.name}) is "
                            f"{prediction_result.get('likelihood_score_percent', 'N/A')}%. No alert sent."
                        )

            except Exception as e_user_predict_notify:
                self.logger.error(
                    f"Error during predictive alert processing for user {user_id}: {e_user_predict_notify}",
                    exc_info=True,
                )
...
async def main_example():
    ...
    class MockAnalyticsService:  # Renamed to avoid conflict if AgentCore's AnalyticsService is also mocked
        ...
        async def predict_incident_likelihood(self, location, prediction_time):
            logger.debug(
                f"Mock predict_incident_likelihood called for {location} at {prediction_time}"
            )
            return {
                "likelihood_score_percent": 75,
                "recommendations": ["Drive carefully"],
            }
        ...
        def get_current_system_kpis_summary(self) -> Dict[str, Any]:
            return {
                ...
                "total_vehicle_flow_rate_hourly": 100,
    ...
    prediction_scheduler_instance = PredictionScheduler(
        analytics_service=analytics_service_mock, prediction_interval_minutes=15
    )
...
if __name__ == "__main__":
    ...
    logger.info(
        "AgentCore module defined. Example main_example() function available for testing (currently commented out)."
    )
