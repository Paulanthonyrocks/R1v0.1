        ...
        while not stop_event.is_set():
            ...
            if result is None or not isinstance(result, tuple) or len(result) != 2:
                if reader.end_of_video:
                    ...
                    logger.debug(
                        f"[{feed_id}] FrameReader.read() returned no frame (queue empty). Retrying..."
                    )
                    if stop_event.is_set():  # Check stop event immediately if no frame
                        logger.info(
                            f"[{feed_id}] Stop event set while waiting for frame. Exiting worker loop."
                        )
                        break
            ...
            if not np.isscalar(current_frame_index):
                logger.warning(
                    f"[{feed_id}] Non-scalar frame index: current_frame_index={current_frame_index}. Skipping frame."
                )
            ...
            if current_frame_index is not None:
                if not (
                    np.isscalar(current_frame_index)
                    and np.isscalar(dynamic_skip_interval)
                ):
                    logger.warning(
                        f"[{feed_id}] Non-scalar frame index or skip interval: current_frame_index={current_frame_index}, dynamic_skip_interval={dynamic_skip_interval}. Skipping frame."
                    )
                    timer.log_time("loop_total", time.time() - loop_start_time)
                    continue
            ...
            try:
                if (
                    frame.shape[1] != target_resolution[0]
                    or frame.shape[0] != target_resolution[1]
                ):
                    processing_frame = cv2.resize(
                        frame, target_resolution, interpolation=cv2.INTER_LINEAR
                    )
                else:
                    processing_frame = frame
            except Exception as e:
                logger.error(
                    f"[{feed_id}] Error preparing/resizing frame {current_frame_index}: {e}. Shape: {frame.shape}. Skip."
                )
            ...
            try:
                ...
                if core_module:
                    ...
                    logger.error(
                        f"[{feed_id}] CoreModule not initialized, cannot process frame {current_frame_index}."
                    )
                ...
                logger.error(
                    f"[{feed_id}] Core Error frame {current_frame_index}: {core_err}",
            ...
            if feed_config_info:
                if (
                    hasattr(feed_config_info, "latitude")
                    and feed_config_info.latitude is not None
                ):
                    metrics["latitude"] = feed_config_info.latitude
                if (
                    hasattr(feed_config_info, "longitude")
                    and feed_config_info.longitude is not None
                ):
            ...
            if combined_frame is None:
                logger.warning(
                    f"[{feed_id}] Visualization returned None for frame {current_frame_index}. Using processing frame."
                )
            ...
            try:
                logger.debug(
                    f"[{feed_id}] Attempting to put frame {current_frame_index} onto queue."
                )
                if frame_queue.full():
                    try:
                ...
                logger.debug(
                    f"[{feed_id}] Successfully put frame {current_frame_index} onto queue."
                )
            except queue.Full:
                logger.error(
                    f"[{feed_id}] Output frame queue STILL FULL after drop attempt! Frame {current_frame_index} lost."
                )
            except Exception as q_put_err:
                logger.error(
                    f"[{feed_id}] Error putting frame {current_frame_index} onto queue: {q_put_err}"
                )
