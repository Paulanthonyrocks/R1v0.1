...
class CoreModule:
    ...
    def detect_and_track(
        ...
        if external_detections is not None:
            raw_detections = external_detections
        else:
            ...
            for det in detections:
                ...
                if self.roi_mask_gpu is not None:
                    ...
                    if cx2 > cx1 and cy2 > cy1:
                        ...
                        if (overlap_area / box_area) < 0.3: continue
                    else: continue
        ...
        # Enforce temporal fusion if not already done by engine
                    ...
                    if not should_run_reid and selected_ids:
                        for tid, track in self.tracker.vehicle_data.items():
                            if tid in selected_ids and self.tracker._bbox_iou(bbox, track["bbox"]) > 0.5:
                                ...
                                break
                    if not is_selected_or_scheduled: continue
        ...
        for tid, track in self.vehicle_data.items():
            ...
            if self.local_ocr and track["status"] == "active" and "license_plate" not in track:
                if frame_index % 10 == 0 and self._ocr_pending_count < 4:
    ...
    def _save_snapshot(self, frame: np.ndarray, incident_id: str):
        try:
            ...
            if fmt not in ["jpg", "jpeg", "webp", "png"]: fmt = "webp"
            ...
            else: cv2.imwrite(str(filepath), frame)
            if self.db_queue:
    ...
    def _run_ocr_task(self, tid, roi):
        try:
            plate_result = None
            if self.local_ocr:
                plate_result = self.local_ocr.read_plate(roi)
            if plate_result:
    ...
    def _estimate_speed_kalman(self, track: Dict, current_time: float, prev_time: float) -> float:
        ...
        try:
            ...
            if prev_smoothed == 0.0 and raw_speed_kmph > 0: new_smoothed = raw_speed_kmph
            else: new_smoothed = (self.ewma_alpha * raw_speed_kmph) + ((1 - self.ewma_alpha) * prev_smoothed)
            speed_err = 0.0
            if kf.P is not None:
                v_var = (kf.P[4, 4] + kf.P[5, 5])
                dynamic_ppm = self._get_dynamic_pixels_per_meter(cy)
                if dynamic_ppm > 0: speed_err = (np.sqrt(v_var) / dynamic_ppm) * 3.6
        ...
        try:
            while True:
                ...
                if tid in self.vehicle_data:
                    ...
                    track["plate_candidates"].append((text, result["confidence"]))
    ...
    def _estimate_lane(self, centroid: Tuple[float, float]) -> int:
        if not centroid or len(centroid) < 2: return -1
        cx, cy = centroid
        ...
        for i in range(len(boundaries) - 1):
            if boundaries[i] <= cx <= boundaries[i+1]: return i + 1 
        return -1

    def _calculate_image_quality(self, roi: np.ndarray, is_occluded: bool = False) -> float:
        if roi.size == 0 or is_occluded: return 0.0
        try:
            gray = cv2.cvtColor(roi, cv2.COLOR_BGR2GRAY)
            sharpness = cv2.Laplacian(gray, cv2.CV_64F).var()
            h, w = roi.shape[:2]
            res_score = min(h, w) / 32.0 
            return sharpness * res_score
        except Exception: return 0.0

    def cleanup(self):
        if self.ocr_executor: self.ocr_executor.shutdown(wait=True)
    ...
    def _preprocess_frame(self, frame: np.ndarray) -> Tuple[np.ndarray, bool, int, int]:
        ...
        if x2 <= x1 or y2 <= y1: return frame, False, 0, 0
        return frame[y1:y2, x1:x2], True, x1, y1

    def update_config(self, updates: Dict[str, Any]):
        allowed_keys = ["name", "latitude", "longitude", "static_object_filter_enabled", "static_object_timeout"]
        for key in allowed_keys:
            if key in updates: self.config[key] = updates[key]

        if "vehicle_detection" in updates:
            v_cfg = updates["vehicle_detection"]
            if "vehicle_detection" not in self.config: self.config["vehicle_detection"] = {}
            ...
            if isinstance(roi_data, list):
                ...
                for pt in roi_data:
                    if isinstance(pt, dict) and 'x' in pt and 'y' in pt:
                        ...
                        if pt[0] <= 1.0 and pt[1] <= 1.0: new_points.append([pt[0] * res[0], pt[1] * res[1]])
                        else: new_points.append(list(pt))
                if new_points:
                    self.roi_polygon_points = new_points
                    if "roi_processing" not in self.config: self.config["roi_processing"] = {}
        ...
        if roi_updated:
            ...
            if self.roi_polygon_points: self.detector.initialize_roi(current_res, self.roi_polygon_points)
