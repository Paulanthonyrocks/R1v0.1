...
class CoreModule:
    ...
    def __init__(self, feed_id: str, gemini_api_key: str, model_path: str, config: Dict, fps: int, db_queue: Optional[MPQueue]):
        ...
        frame_w, _ = vehicle_detection_cfg.get('frame_resolution', [640, 480]) # frame_h not used

        lane_detection_cfg = self.config.get('lane_detection', {})
        self.num_lanes = lane_detection_cfg.get('num_lanes', 6)
        default_lane_width = frame_w / (self.num_lanes + 1) if self.num_lanes > 0 else frame_w / 4
        self.lane_width_pixels = lane_detection_cfg.get('lane_width', default_lane_width)
        self.lane_change_buffer = lane_detection_cfg.get('lane_change_buffer', 5)

        self.lane_width_meters = 3.7
        default_ppm = self.lane_width_pixels / self.lane_width_meters if self.lane_width_meters > 0 else 50
        self.pixels_per_meter = self.config.get('pixels_per_meter', default_ppm)
        self.speed_limit = self.config.get('speed_limit', 60)

        # Perspective matrix loading
        ...
        if matrix_path_str:
            ...
            if matrix_path.exists():
                ...
                logger.warning(f"Perspective matrix path not found: {matrix_path}")
    ...
    def _load_model(self, use_gpu: bool):
        ...
        try:
            ...
            raise ImportError("PyTorch/Ultralytics is required.") from e
        except Exception as e:
    ...
    def detect_and_track(self, frame: np.ndarray, frame_index: int,
        ...
        try:
            ...
            self._save_vehicle_data(current_tracks)
            return current_tracks
        except Exception as e:
            logger.error(f"Frame {frame_index}: Error in detect_and_track: {e}", exc_info=True)
            return {}

    def _detect_vehicles(self, frame: np.ndarray, frame_index: int, confidence_threshold: float) -> List[Tuple]:
        detections = []
        try:
            img_size = self.yolo_imgsz if isinstance(self.yolo_imgsz, int) and self.yolo_imgsz > 0 else 640
            results = self.model.predict(frame, conf=confidence_threshold, imgsz=img_size,
                                         classes=self.vehicle_class_ids,
                                         max_det=self.max_active_tracks, verbose=False)
            for r in results:
                boxes = r.boxes
                for box in boxes:
                    if not all(hasattr(box, attr) for attr in ['conf', 'xyxy', 'cls']):
                        continue
                    ...
                    detections.append(
                        (center_x, center_y, conf, cls, frame_index, vehicle_bbox))
            return detections

        except Exception as e:
            logger.error(
                f"Frame {frame_index}: Error during YOLO detection: {e}", exc_info=True)
            return []
    ...
    def _update_tracks(self, frame: np.ndarray, detections: List[Tuple], proximity_threshold: int,
        ...
        for vehicle_id in track_ids_to_match:
            ...
            if kf:
                ...
                dt = min(1.0, max(0.01, current_time -
                         track.get('last_seen', current_time)))  # Bounded dt
                kf.F[0, 2] = dt
                kf.F[1, 3] = dt
                kf.predict()
            ...
            try:
                ...
                for i, vehicle_id in enumerate(track_ids_to_match):
                    ...
                    if distance < proximity_threshold and best_match_idx not in matched_detection_indices:
                        ...
                        track['kalman_filter'] = kalman_filters[i]
                ...
                logger.error(
                    f"Error during KDTree matching: {tree_err}", exc_info=True)

        unmatched_detections_indices = set(
    ...
    def _update_track(self, track: Dict, detection: Tuple, current_time: float, frame: np.ndarray, frame_index: int) -> None:
        try:
            center_x, center_y, conf, _, vehicle_bbox = detection # class_id not used here
            prev_time = track['last_seen']
            track.update({
                'last_seen': current_time, 'timestamp': current_time, 'frame_index': frame_index,
                'bbox': vehicle_bbox, 'confidence': conf
            })
            ...
            # center_lane_new = (new_lane - 0.5) * self.lane_width_pixels # F841
            center_lane_old = (last_recorded_lane - 0.5) * self.lane_width_pixels if last_recorded_lane != -1 else center_x

            if last_recorded_lane != -1 and new_lane != -1 and new_lane != last_recorded_lane and \
               abs(center_x - center_lane_old) > self.lane_change_buffer:
            ...
            max_ocr_attempts = self.ocr_cfg.get('max_ocr_attempts', 3) # Use config
            plate_unknown = track['license_plate'] == "Unknown"
            ocr_attempt_due = frame_index % max(1, ocr_interval_frames) == 0
            can_attempt_ocr = track.get('plate_attempts', 0) < max_ocr_attempts

            if plate_unknown and self.preprocessor and can_attempt_ocr and ocr_attempt_due:
                ...
                unknown_responses = ["Unknown", "Unknown (Error)", "Unknown (BadROI)", "Unknown (SmallROI)",
                                     "Unknown (NoPrep)", "Unknown (RetryFail)", "Unknown (Refused)",
                                     "Unknown (Blocked)", "Unknown (GenFail)", "Unknown (InvalidResp)",
                                     "Unknown (OCRError)", "Unknown (PreprocFail)", "Unknown (TessFail)",
                                     "Unknown (NoTess)", "Unknown (TessError)", None]
                if plate_text not in unknown_responses:
    ...
    def _classify_behavior(self, track: Dict) -> None:
        current_speed_kmh = track.get('speed', 0.0) # Ensure speed exists
    ...
    def _estimate_speed_kalman(self, track: Dict, current_time: float, prev_time: float) -> float:
        ...
        try:
            ...
            time_diff = min(1.0, max(0.01, current_time - prev_time))
            pixel_speed_per_sec = np.sqrt(
                vx**2 + vy**2) / time_diff if time_diff > 0 else 0
            speed_mps = pixel_speed_per_sec / \
                self.pixels_per_meter if self.pixels_per_meter > 0 else 0
            ...
            current_history.append(speed_kmph)
    ...
    def _ocr_license_plate(self, frame: np.ndarray, bbox: List[int]) -> str:
        ...
        try:
            ...
            top_margin_factor = self.ocr_cfg.get(
                'roi_top_margin_factor', 0.5)  # Default: Start lower
            ...
            left_margin_factor = self.ocr_cfg.get(
                'roi_left_margin_factor', 0.15)  # Default: Crop left side
            right_margin_factor = self.ocr_cfg.get(
                'roi_right_margin_factor', 0.15)  # Default: Crop right side
            ...
            if roi.size < self.preprocessor.min_roi_size:
    ...
    def _initialize_kalman_filter(self, initial_x: float, initial_y: float) -> KalmanFilter:
        try:
            ...
            kf.F = np.array([[1, 0, 1, 0], [0, 1, 0, 1], [0, 0, 1, 0], [
                            0, 0, 0, 1]], dtype=float)  # dt=1 initially
    ...
    def _estimate_lane(self, bbox: List[int]) -> int:
        if not bbox or len(bbox) != 4 or self.lane_width_pixels <= 0:
            return -1
        x_center = (bbox[0] + bbox[2]) / 2
        lane = int(x_center // self.lane_width_pixels) + 1
        return max(1, min(lane, self.num_lanes))

    def _remove_stale_tracks(self, current_time: float, track_timeout: int) -> None:
        stale_ids = [
            vid for vid, track in self.vehicle_data.items()
            if current_time - track.get('last_seen', current_time) > track_timeout # Use .get for safety
        ]
        for vid in stale_ids:
            if vid in self.vehicle_data: # Check existence before deleting
                del self.vehicle_data[vid]

        active_track_count = len(self.vehicle_data)
        if active_track_count > self.max_active_tracks:
            # Sort by last_seen, oldest first
            sorted_tracks = sorted(self.vehicle_data.items(), key=lambda item: item[1].get('last_seen', 0))
            num_to_remove = active_track_count - self.max_active_tracks
            for i in range(num_to_remove):
                vid_to_remove = sorted_tracks[i][0]
                if vid_to_remove in self.vehicle_data: # Check existence
                    del self.vehicle_data[vid_to_remove]
        if stale_ids or active_track_count > self.max_active_tracks: # Log if any action was taken
            logger.debug(f"Removed stale/excess. Active tracks: {len(self.vehicle_data)}")

    def _save_vehicle_data(self, current_tracks: Dict[str, Dict]) -> None:
        if not self.db_queue or not current_tracks:
            return

        vehicle_data_list = []
        for track_id, track_data in current_tracks.items():
            if not track_data or 'bbox' not in track_data: # Ensure track_data and bbox exist
                logger.warning(f"Skipping DB save for incomplete track data: {track_id}")
                continue
            try:
                vehicle_data = {
                    'vehicle_id': track_id,
                    'timestamp': track_data.get('timestamp', time.time()),
                    'frame_index': track_data.get('frame_index'),
                    'license_plate': track_data.get('license_plate', 'Unknown'),
                    'vehicle_type': self._get_vehicle_type(track_data.get('class_id', -1)),
                    'first_seen': track_data.get('first_seen'),
                    'last_seen': track_data.get('last_seen'),
                    'x1': track_data['bbox'][0], 'y1': track_data['bbox'][1],
                    'x2': track_data['bbox'][2], 'y2': track_data['bbox'][3],
                    'speed': track_data.get('speed'),
                    'lane': track_data.get('lane'),
                ...
                logger.warning(f"KeyError for {track_id}: {ke}. Skipping DB save.")
            except Exception as e:
                logger.error(f"Error preparing DB data for {track_id}: {e}", exc_info=True)

        if not vehicle_data_list:
            return

        try:
            for item in vehicle_data_list: # Iterate over list directly
                self.db_queue.put_nowait(item)
        except queue.Full:
            logger.warning("DB queue full. Dropping vehicle data batch.")
        except Exception as e:
            logger.error(f"Failed to put data onto db_queue: {e}", exc_info=True)

    def _get_vehicle_type(self, class_id: int) -> str:
        type_map = {2: 'car', 3: 'motorcycle', 5: 'bus', 7: 'truck'} # Corrected type_map
...
if __name__ == "__main__":
    ...
    try:
        ...
        for i in range(5):
            ...
            if i == 1:
                cv2.rectangle(dummy_frame, (100, 100), (150, 150),
                              (0, 255, 0), -1)  # Add a "vehicle"
            if i == 2:
                cv2.rectangle(dummy_frame, (110, 110), (160, 160),
                              (0, 255, 0), -1)  # Move it
            if i == 3:
                cv2.rectangle(dummy_frame, (200, 200), (250, 250),
                              (0, 0, 255), -1)  # Add another
