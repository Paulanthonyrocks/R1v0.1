...
class PersonalizedRoutingService:
    ...
    async def initialize_tables(self):
        ...
        if self._db_manager.async_engine:
            ...
            logger.info("PersonalizedRoutingService tables created/checked successfully.")
        else:
            logger.error("Async database engine not available for PersonalizedRoutingService table initialization.")
            raise RuntimeError("Database async engine not configured.")

    async def _get_most_frequent_destination(self, user_id: str) -> Optional[Dict[str, Any]]:
        """
        ...
        async with self._db_manager.get_session() as session:
            try:
                ...
                stmt = select(RouteHistoryModel).filter(RouteHistoryModel.user_id == user_id)
                ...
                for record in history_records:
                    end_loc_str = str(record.end_location) # Convert dict to string for hashing
                    if end_loc_str:
                        destination_counts[end_loc_str] = destination_counts.get(end_loc_str, 0) + 1

                if not destination_counts:
                    return None

                # Find the most frequent destination
                most_frequent_dest_str = max(destination_counts, key=destination_counts.get)
                ...
                logger.error(f"Database error getting most frequent destination for user {user_id}: {e}")
                return None
            except Exception as e:
                logger.error(f"Unexpected error getting most frequent destination for user {user_id}: {e}", exc_info=True)
                return None
    ...
    async def proactively_suggest_route(self, user_id: str) -> Optional[str]:
        ...
        async with self._db_manager.get_session() as session:
            try:
                most_common_destination = await self._get_most_frequent_destination(user_id)
                ...
                stmt = select(ProactiveSuggestionFeedbackLog).filter(
    ...
    async def record_suggestion_feedback(
        ...
        async with self._db_manager.get_session() as session:
            try:
                stmt = select(ProactiveSuggestionFeedbackLog).filter_by(suggestion_id=suggestion_id)
