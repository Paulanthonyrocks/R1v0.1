...
class RealtimeMetricsUpdate(BaseModel):
    feed_id: str = Field(..., description="ID of the feed generating metrics")
    timestamp: datetime = Field(default_factory=datetime.utcnow,
                                description="Timestamp of metrics update (UTC)")
    metrics: Dict[str, Any] = Field(..., description="Key-value pairs of metrics",
...
class GlobalRealtimeMetrics(BaseModel):
    ...
    metrics_source: Optional[str] = Field(
        None, description="Source of metrics (e.g., 'FeedManagerGlobalKPIs')"
    )
    congestion_index: Optional[float] = Field(
        None, example=45.5, description="Overall congestion index for monitored area"
    )
    average_speed_kmh: Optional[float] = Field(
        None, example=30.2, description="Average speed across relevant feeds/areas"
    )
...
class GeneralNotification(BaseModel):
    ...
    message: str = Field(..., example="System maintenance: Tonight 2 AM - 3 AM UTC.")
    severity: str = Field(default="info", example="info|warning|error",
...
class ErrorNotification(BaseModel):
    ...
    details: Optional[str] = Field(None, description="Additional technical error details")
    timestamp: datetime = Field(default_factory=datetime.utcnow)
...
class WebSocketMessage(BaseModel):
    event_type: WebSocketMessageTypeEnum = Field(
        ..., description="The type of event this message represents")
    payload: Union[
        RealtimeMetricsUpdate, GlobalRealtimeMetrics, NewAlertNotification,
        SignalStateUpdate, FeedStatusUpdate, GeneralNotification,
        ErrorNotification, Dict[str, Any]  # For simple payloads
    ]
    client_id: Optional[str] = Field(
        None, description="Target client ID (None for broadcast)"
    )
    correlation_id: Optional[str] = Field(
        None, description="Optional ID to correlate requests/responses"
    )
