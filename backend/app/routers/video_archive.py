...
async def start_video_recording(
    ...
    if success:
        return {"message": f"Recording started for stream {request.stream_id}", "output_path": os.path.join("backend", "data", "processed_videos", request.output_filename)}
    raise HTTPException(status_code=400, detail=f"Failed to start recording for stream {request.stream_id}")
...
async def stop_video_recording(
    ...
    processor = video_manager.get_processor(stream_id, Depends(get_feed_manager)) # Feed manager is needed for processor instance
    success = await processor.stop_recording()
    if success:
        return {"message": f"Recording stopped for stream {stream_id}"}
    raise HTTPException(status_code=400, detail=f"Failed to stop recording for stream {stream_id}")
