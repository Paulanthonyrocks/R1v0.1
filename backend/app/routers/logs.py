...
async def get_system_logs(
    ...
    limit: int = 100,
    offset: int = 0,
    current_user: Dict[str, Any] = Depends(get_current_active_user) # Protect with authentication
) -> List[Dict[str, Any]]:
    """
    ...
    if not target_log_file.exists():
        ...
        raise HTTPException(status_code=404, detail=f"Log file '{log_file_name}' not found.")
        ...
        with open(target_log_file, "r", encoding="utf-8", errors="ignore") as f:
            ...
            for i in range(start_index, end_index):
                ...
                if line:
                    ...
                    match = re.match(r"^(?P<timestamp>\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2},\d{3}) - (?P<name>[\w.]+) - (?P<level>\w+) - (?P<message>.*)$", line)
                    if match:
