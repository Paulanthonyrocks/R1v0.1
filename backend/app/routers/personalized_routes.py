...
async def get_personalized_route(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error getting personalized route: {str(e)}")
...
async def record_route_history(
    ...
    try:
        ...
        if entry.user_id != current_user['uid']:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="User ID in entry does not match authenticated user")
            
        await routing_service.record_route_history(entry)
        return {"message": "Route history recorded successfully"}
        
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error recording route history: {str(e)}")
...
async def get_user_profile(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error getting user profile: {str(e)}")
...
async def get_route_history(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error getting route history: {str(e)}")
...
async def record_suggestion_feedback_endpoint(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Suggestion ID not found, user mismatch, or invalid data. Feedback not recorded.")
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"An unexpected error occurred while recording feedback: {str(e)}")
