...
class TrafficMonitor:
    vehicle_type_map: Dict[int, str] = {
        2: "car", 3: "motorcycle", 5: "bus", 7: "truck", -1: "unknown",
    }

    def __init__(self, config: Dict[str, Any], feed_id: str):
        self.config = config
        self.feed_id = feed_id  # BUG #22 FIX: Store feed_id
        ...
        self.session_metrics = {
            "speed_sum": 0.0, "speed_samples": 0, "congestion_sum": 0.0,
            "congestion_samples": 0, "congestion_score_sum": 0.0, "congestion_score_samples": 0
        }
    ...
    def update_vehicles(self, vehicles: Dict[str, Dict[str, Any]]):
        ...
        for track_id, data in vehicles.items():
            ...
            if not data.get("centroid"): continue
            ...
            if lane != -1:
                ...
                if speed is not None: self.lane_speeds[lane].append(speed)
        ...
        if len(self.anomalies) > 100: self.anomalies = self.anomalies[-100:]
        ...
        for v_id, data in vehicles.items():
            ...
            if accel < -8.0:
                self.anomalies.append({
                    "type": "hard_braking", "vehicle_id": v_id, "timestamp": now, "severity": "Warning",
                    "details": f"Sudden deceleration detected: {accel:.1f} m/s²", "location": data.get("centroid")
                })
            direction = data.get("direction")
            if direction == "North" and data.get("vy", 0) > 2.0:
                 self.anomalies.append({
                    "type": "wrong_way", "vehicle_id": v_id, "timestamp": now, "severity": "Critical",
                    "details": "Vehicle traveling against lane flow", "location": data.get("centroid")
                })

    def get_metrics(self) -> Dict[str, Any]:
        current_vehicle_count = len(self.tracked_vehicles)
        stopped_count, speeding_count = 0, 0
        speeds_list_kmh: list[float] = []
        vehicle_type_counts: Dict[str, int] = {name: 0 for name in self.vehicle_type_map.values()}
        if "unknown" not in vehicle_type_counts: vehicle_type_counts["unknown"] = 0
        ...
        if current_vehicle_count > 0:
            ...
            if self.smoothed_congestion_score == 0.0: self.smoothed_congestion_score = raw_congestion_score
            else: self.smoothed_congestion_score = (self.congestion_alpha * raw_congestion_score) + ((1 - self.congestion_alpha) * self.smoothed_congestion_score)
            ...
            if self.smoothed_congestion_score < 0.1: self.smoothed_congestion_score = 0.0
            congestion_score = round(self.smoothed_congestion_score, 1)
        ...
        for v in self.tracked_vehicles.values():
            lid = v.get("lane", -1)
            if 1 <= lid <= num_lanes: current_lane_counts[lid] += 1
        ...
        for i in range(1, num_lanes + 1):
            ...
            if len(stopped_in_lane) >= 3: lane_queues[i] = len(stopped_in_lane) * 6.0
            speeds = list(self.lane_speeds.get(i, []))
            if len(speeds) >= 5:
                lane_speed_profiles[i] = {
                    "avg": round(float(np.mean(speeds)), 1), "p85": round(float(np.percentile(speeds, 85)), 1),
                    "std": round(float(np.std(speeds)), 1)
                }
            else: lane_speed_profiles[i] = {"avg": 0.0, "p85": 0.0, "std": 0.0}
        ...
        return {
            "feed_id": self.feed_id, # BUG #22 FIX: Include feed_id in metrics
            ...
            "stopped_vehicles": stopped_count, "speeding_vehicles": speeding_count,
            "average_speed_kmh": round(avg_speed_kmh, 1),
            "congestion_level_percent": round(congestion_lvl_percent, 1),
            "is_congested": is_congested, "congestion_score": congestion_score,
            "vehicles_per_lane": current_lane_counts, "high_density_lanes": high_density_lanes,
            "vehicle_type_counts": vehicle_type_counts, "lane_occupancy": lane_occupancy,
            "queue_lengths": lane_queues, "lane_speed_profiles": lane_speed_profiles,
            ...
            "anomalies": self.anomalies[-5:], "health_score": round(avg_health, 1), "health_status": health_status
        }

    def _calculate_gap_and_headway(self, num_lanes: int) -> Tuple[Dict[int, float], Dict[int, float]]:
        lane_gaps, lane_headways = {}, {}
        now = time.time()
        for lane_id in range(1, num_lanes + 1):
            lane_vehicles = []
            for v in self.tracked_vehicles.values():
                if v.get("lane") == lane_id:
                    if "centroid" not in v and "bbox" in v: b = v["bbox"]; v["centroid"] = [(b[0] + b[2]) / 2, (b[1] + b[3]) / 2]
            ...
            for i in range(len(lane_vehicles) - 1):
                v_front, v_back = lane_vehicles[i+1], lane_vehicles[i]
                p1, p2 = v_front.get("ground_coordinates"), v_back.get("ground_coordinates")
                if p1 and p2:
                    ...
                    if dist > 0:
                        ...
                        if speed > 5.0:
                            ...
                            if headway < 1.0:
                                self.anomalies.append({
                                    "type": "tailgating", "vehicle_id": v_back.get("vehicle_id", "unknown"), "timestamp": now,
                                    "severity": "Warning", "details": f"Tailgating detected: Headway {headway:.2f}s",
