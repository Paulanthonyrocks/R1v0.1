...
import logging
import sqlite3
import queue # Used by FrameReader
import threading
...
# from multiprocessing import Queue as MPQueue # F401: Unused import (using threading.Queue)
...
def check_system_resources(cpu_interval: float = 0.1) -> Tuple[float, float]:
    ...
    try:
        ...
        logger.error(
            f"Failed to get system resource usage: {e}", exc_info=True)
        return 0.0, 0.0
...
DEFAULT_CONFIG: Dict[str, Any] = {
    ...
    "ocr_engine": {
        ...
        "ocr_interval": 10,
        "gemini_api_key": None,
        # Note: tenacity @retry stop_after_attempt overrides this if different
...
def load_config(config_path: str) -> Dict[str, Any]:
    ...
    if not path.is_file():
        logger.warning(
            f"Configuration file not found at {path}. Using default configuration ONLY.")
        ...
        try:
            with open(path, 'r') as f:
                ...
                if not isinstance(loaded_config, dict):
                    if loaded_config is None:
                        logger.warning(
                            f"Configuration file {path} is empty. Using default configuration.")
                        loaded_config = {}
                    else:
                        raise ConfigError(
                            f"Configuration file {path} is not a valid YAML dictionary.")
                config = DEFAULT_CONFIG.copy()
            ...
            logger.error(
                f"An unexpected error occurred loading configuration file {path}: {e}", exc_info=True)
            raise ConfigError(
                f"Unexpected error loading configuration: {e}") from e
    ...
    try:
        ...
        if api_key is not None and (not isinstance(api_key, str) or not api_key.strip()):
            raise ConfigError(
                f"Invalid 'ocr_engine.gemini_api_key'. Must be a non-empty string or null/None.")

        db_path_str = config["database"]["db_path"]
        if not isinstance(db_path_str, str) or not db_path_str.strip():
            raise ConfigError(
                f"'database.db_path' must be a non-empty string. Got: '{db_path_str}'")
        ...
        if not db_file_path.parent.exists():
            try:
                logger.info(
                    f"Creating database directory: {db_file_path.parent}")
                db_file_path.parent.mkdir(parents=True, exist_ok=True)
            except OSError as e:
                raise ConfigError(
                    f"Failed to create database directory {db_file_path.parent}: {e}") from e
        ...
        if not model_file_path.is_file():
            raise ConfigError(
                f"Vehicle detection model file not found at specified/resolved path: '{model_file_path}'")
        ...
        logger.error(
            f"An unexpected error occurred during configuration validation: {e}", exc_info=True)
        raise ConfigError(
            f"Unexpected error validating configuration: {e}") from e
...
class FrameReader:
    def __init__(self, source: Any, buffer_size: int = 2, target_fps: Optional[int] = None):
        ...
        if not self.cap.isOpened():
            logger.error(
                f"FrameReader: Failed to open video source: {capture_source} (from {self.source_name})")
            raise RuntimeError(f"Cannot open video source: {capture_source}")
        logger.info(
            f"FrameReader: Successfully opened video source: {capture_source} (from {self.source_name})")
        ...
        logger.info(
            f"Source properties: {self.source_width}x{self.source_height} @ {self.source_fps:.2f} FPS")
        ...
        if self.is_webcam:
            ...
            if self.target_fps:
                ...
                logger.info(
                    f"Attempting to set webcam FPS to: {self.target_fps}")
        elif self.target_fps and self.target_fps != self.source_fps:
            logger.warning(
                f"Target FPS {self.target_fps} diffs from source {self.source_fps}. Frame drop/dup may occur.")
        ...
        self.thread = threading.Thread(
            target=self._update_loop, daemon=True, name=f"FrameReader-{self.source_name}")
        self.frame_index = 0
        self.thread.start()
                ...
                if ret:
                    ...
                    if self.frame_queue.full():
                        try:
                            ...
                            logger.warning(
                                f"FR queue full {self.source_name}. Discard oldest.")
                        except queue.Empty:
                            pass
                    try:
                        ...
                        logger.warning(
                            f"FR queue still full. Frame {self.frame_index} lost.")
                else:
                    consecutive_fails += 1
                    logger.warning(
                        f"FR {self.source_name}: cv2.read() False (Fail {consecutive_fails}/{max_read_fails}).")
                    if consecutive_fails >= max_read_fails:
                        logger.error(
                            f"FR {self.source_name}: Max read fails. End of video/HW issue.")
                    ...
                    if not self.is_webcam and self.cap.get(cv2.CAP_PROP_POS_FRAMES) >= self.cap.get(cv2.CAP_PROP_FRAME_COUNT):
                        logger.info(
                            f"FR {self.source_name}: Reached end of video file.")
                        self.end_of_video = True
                        break
            except Exception as e:
                logger.error(
                    f"FR thread error ({self.source_name}): {e}", exc_info=True)
                self.end_of_video = True
                break
        logger.info(f"FR thread stopping for {self.source_name}.")
        self.end_of_video = True
        if self.cap and self.cap.isOpened():
            self.cap.release()
            logger.info(f"FR video capture released for {self.source_name}.")
    ...
    def read(self) -> Optional[Tuple[int, np.ndarray]]:
        try:
            ...
            if self.end_of_video and (not self.thread.is_alive() or self.frame_queue.empty()):
                logger.debug(
                    f"FR {self.source_name}: Read, queue empty & EOV.")
                return None
            logger.debug(f"FR {self.source_name}: Read, queue temp empty.")
            return None

    def stop(self):
        logger.info(f"FR {self.source_name}: Stop requested.")
        self.stop_event.set()
        if self.thread.is_alive():
            self.thread.join(timeout=2.0)
            if self.thread.is_alive():
                logger.warning(
                    f"FR thread {self.source_name} did not exit cleanly.")
        if self.cap and self.cap.isOpened():
            self.cap.release()
...
class TrafficMonitor:
    vehicle_type_map = {2: 'car', 3: 'motorcycle',
                        5: 'bus', 7: 'truck'}  # Class attribute
    ...
    def __init__(self, config: Dict):
        ...
        self.congestion_speed_threshold = incident_cfg.get(
            'congestion_speed_threshold', 20)
        self.stopped_threshold_kmh = config.get(
            'stopped_speed_threshold_kmh', 5)
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        for data in self.tracked_vehicles.values():
            ...
            type_name = self.vehicle_type_map.get(
                data.get('class_id', -1), 'unknown')
            vehicle_type_counts[type_name] += 1
        avg_speed_kmh = float(np.mean(speeds_list_kmh)
                              ) if speeds_list_kmh else 0.0
        congestion_lvl = float(
            (stopped_count / total_vehicles * 100.0)) if total_vehicles > 0 else 0.0
        is_congested = avg_speed_kmh < self.congestion_speed_threshold and total_vehicles > self.density_threshold
        high_density_lanes = [lane for lane, count in self.lane_counts.items(
...
def create_lane_overlay(shape: Tuple[int, int, int], num_lanes: int, lane_width: float, density_per_lane: Dict[int, int], config: Dict) -> np.ndarray:
    ...
    threshold_medium = threshold_high // 2
    levels = {'low': (0, 255, 0, 60), 'medium': (
        255, 165, 0, 80), 'high': (255, 0, 0, 100)}
...
def create_grid_overlay(shape: Tuple[int, int, int], config: Dict) -> np.ndarray:
    ...
    lanes = config.get('lane_detection', {}).get('num_lanes', 0)
    grid_interval_pixels = int(10 * ppm) if ppm > 0 else 100
    grid_color = (100, 100, 100, 80)
    for y_coord in range(grid_interval_pixels, h, grid_interval_pixels):
        cv2.line(overlay, (0, y_coord), (w, y_coord),
                 grid_color, 1, cv2.LINE_AA)
    if lanes > 0:
        lane_width_pixels = w / lanes
        for i in range(1, lanes):
            cv2.line(overlay, (int(i * lane_width_pixels), 0),
                     (int(i * lane_width_pixels), h), grid_color, 1, cv2.LINE_AA)
    return overlay
...
def alpha_blend(foreground: np.ndarray, background: np.ndarray) -> np.ndarray:
    if foreground.shape[:2] != background.shape[:2]:
        foreground = cv2.resize(
            foreground, (background.shape[1], background.shape[0]), interpolation=cv2.INTER_NEAREST)
...
def visualize_data(frame: Optional[np.ndarray], tracked_vehicles: Dict[int, Dict], traffic_metrics: Dict[str, Any], visualization_options: Set[str], config: Dict, feed_id: str = "") -> Optional[np.ndarray]:
    ...
    try:
        ...
        if overlay_cache_size != current_size:
            logger.debug(
                f"[{feed_id}] Frame size changed. Resetting viz overlays.")
            cached_lane_overlay = None
            cached_grid_overlay = None
        ...
        if "Grid Overlay" in visualization_options:
            if cached_grid_overlay is None:
                cached_grid_overlay = create_grid_overlay(
                    vis_frame.shape, config)
            ...
            for veh_id, data in tracked_vehicles.items():
                ...
                if bbox:
                    ...
                    if "Vehicle Data" in visualization_options:
                        lines = [
                            f"ID:{veh_id}({class_name})", f"Spd:{speed:.1f}km/h"]
                        lh = 15
                        if plate:
                            lines.append(f"LP:{plate}")
                        text_y = y1-7 if y1-7 >= lh*len(lines) else y2+lh
                        for i, line_text in enumerate(lines):
                            cv2.putText(vis_frame, line_text, (x1+5, text_y+i*lh),
                                        cv2.FONT_HERSHEY_SIMPLEX, 0.4, color, 1, cv2.LINE_AA)
        banner_h = 25
        banner_text = f"{time.strftime('%H:%M:%S')} | {feed_id} | Veh:{traffic_metrics.get('total_vehicles',0)} | AvgSpd:{traffic_metrics.get('average_speed_kmh',0.0):.1f}"
        if traffic_metrics.get('is_congested', False):
            banner_text += " | CONGESTED"
        cv2.rectangle(vis_frame, (0, 0), (w, banner_h), (0, 0, 0, 180), -1)
        cv2.putText(vis_frame, banner_text, (10, banner_h-8),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
        return vis_frame
    except Exception as e:
        logger.error(f"[{feed_id}] Viz error: {e}", exc_info=True)
        return frame
...
class LicensePlatePreprocessor:
    ...
    def __init__(self, config: Dict, perspective_matrix: Optional[np.ndarray] = None):
        ...
        self.morph_kernel = np.array(self.config.get(
            "morph_kernel", [[1, 1, 1], [1, 1, 1], [1, 1, 1]]), dtype=np.uint8)
        self.sharpen_kernel = np.array(self.config.get(
            "sharpen_kernel", [[-1, -1, -1], [-1, 9, -1], [-1, -1, -1]]), dtype=np.float32)

    @retry(
        wait=wait_exponential(multiplier=1, min=2, max=10),
        stop=stop_after_attempt(3),
        retry=retry_if_exception_type((
            # Covers API key invalid, not enabled, permissions
            google_api_exceptions.PermissionDenied,
            # This generally covers rate limits / quota exceeded
    ...
    def _call_gemini_ocr(self, image_roi: np.ndarray) -> str:
        ...
        logger.debug(
            f"Attempting Gemini OCR call for ROI of shape {image_roi.shape}")
        try:
            pil_image = Image.fromarray(
                cv2.cvtColor(image_roi, cv2.COLOR_BGR2RGB))
            ...
            if ocr_text:
                ...
                logger.debug("Gemini OCR: No plate found or empty result.")
        ...
        # Handle API errors that tenacity should retry
            ...
            # Set cool-down for after retries fail
            self.last_api_error_time = time.monotonic()
            raise  # Re-raise for tenacity to catch and retry
        except Exception as e:
            logger.error(
                f"Unexpected error during Gemini OCR call: {e}", exc_info=True)
            self.last_api_error_time = time.monotonic()
    ...
    def _preprocess_for_tesseract(self, roi: np.ndarray) -> Optional[np.ndarray]:
        ...
        try:
            ...
            logger.error(
                f"Error in _preprocess_for_tesseract: {e}", exc_info=True)
            return None
    ...
    def preprocess_and_ocr(self, roi: np.ndarray) -> str:
        ...
        if self.model and self.gemini_api_key:
            ...
            try:
                ...
                logger.error(
                    f"Gemini OCR failed after all retries: {e.last_attempt.exception()}. ROI shape: {roi.shape}")
                ocr_result = ""  # Ensure it's empty
            except Exception as e:  # Should not happen if _call_gemini_ocr handles its exceptions
                logger.error(
                    f"Unexpected error during Gemini OCR attempt sequence: {e}", exc_info=True)
                ocr_result = ""
        ...
        if not pytesseract:  # Check if pytesseract module itself is available
            if not (self.model and self.gemini_api_key):  # Only log if no OCR was even attempted
                logger.warning(
                    "Pytesseract not available and Gemini not configured. No OCR will be performed.")
        ...
        if processed_roi_for_tesseract is not None:
            try:
                ...
                text = pytesseract.image_to_string(
                    processed_roi_for_tesseract, config=custom_config, timeout=5)  # Add timeout
                ...
                if ocr_result:
                    ...
                    logger.debug(
                        "Tesseract OCR: No text found or empty result.")
            except RuntimeError as e:  # Catches Tesseract not found or timeout
                # exc_info False as message is usually clear
                logger.error(
                    f"Tesseract runtime error (e.g., Tesseract not installed/found, or timeout): {e}", exc_info=False)
                ocr_result = ""
            except Exception as e:
                logger.error(
                    f"Tesseract OCR unexpected error: {e}", exc_info=True)
                ocr_result = ""
        else:
            logger.warning(
                "Preprocessing for Tesseract failed, cannot perform Tesseract OCR.")
...
class DatabaseManager:
    def __init__(self, config: Dict):
        ...
        if not self.db_path:
            raise ConfigError("SQLite 'db_path' not in config.")
        self.chunk_size = self.db_config.get("chunk_size", 100)
        self.lock = threading.RLock() # For SQLite operations
        ...
        if self.mongo_uri and self.mongo_db_name:
            logger.info(f"MongoDB URI found. Initializing connection to {self.mongo_db_name}...")
            self._initialize_mongodb()
        else:
            logger.warning("MongoDB URI or database_name not in config. MongoDB disabled.")
    ...
    def _get_sqlite_connection(self) -> sqlite3.Connection:
        try:
            ...
            try:
                ...
                logger.warning(f"Could not set WAL/sync mode on {self.db_path}: {e}")
            return conn
        except sqlite3.Error as e:
            logger.error(f"Failed to connect to SQLite DB {self.db_path}: {e}", exc_info=True)
            raise DatabaseError(f"SQLite connect fail: {e}") from e
    ...
    def _initialize_sqlite_database(self):
        ...
        try:
            ...
            logger.error(f"SQLite DB init error: {e}", exc_info=True)
            raise DatabaseError(f"SQLite schema init fail: {e}") from e
        except Exception as e: # Catch any other unexpected errors
            logger.error(f"Unexpected SQLite DB init error: {e}", exc_info=True)
            raise DatabaseError(f"Unexpected SQLite DB init error: {e}") from e
    ...
    def _create_sqlite_tables(self, cursor: sqlite3.Cursor):
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS vehicle_tracks (
                ...
                class_id INTEGER, confidence REAL,
                bbox_x1 REAL, bbox_y1 REAL, bbox_x2 REAL, bbox_y2 REAL,
                center_x REAL, center_y REAL, speed REAL, acceleration REAL,
                lane INTEGER, direction REAL, license_plate TEXT,
                ocr_confidence REAL, flags TEXT,
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS alerts (
                ...
                feed_id TEXT NOT NULL, message TEXT NOT NULL, details TEXT,
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS raw_traffic_data (
                ...
                sensor_id TEXT NOT NULL, latitude REAL NOT NULL, longitude REAL NOT NULL,
                speed REAL, occupancy REAL, vehicle_count INTEGER
            )
        ''')
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS processed_traffic_data (
                id INTEGER PRIMARY KEY AUTOINCREMENT, segment_id TEXT NOT NULL,
                timestamp TEXT NOT NULL, congestion_level REAL NOT NULL
            )
        ''')
    ...
    def _initialize_mongodb(self):
        try:
            ...
            self.mongo_db = self.mongo_client[self.mongo_db_name] # type: ignore
            logger.info(f"Successfully connected to MongoDB: '{self.mongo_db_name}'")
            # Example index creation (run once or ensure exists)
            # self.mongo_db[self.raw_traffic_collection_name].create_index(
            #     [("timestamp", -1), ("sensor_id", 1)], background=True
            # )
        except (ConnectionFailure, MongoConfigurationError) as e:
            logger.error(f"MongoDB connection/config failed for {self.mongo_uri}: {e}", exc_info=True)
            self.mongo_client = None
            self.mongo_db = None
        except Exception as e: # Catch other unexpected errors
            logger.error(f"Unexpected MongoDB init error for {self.mongo_uri}: {e}", exc_info=True)
            self.mongo_client = None
            self.mongo_db = None
    ...
    def save_vehicle_data(self, vd: Dict) -> bool:
        sql = '''
            INSERT OR REPLACE INTO vehicle_tracks (
                ...
                speed, acceleration, lane, direction, license_plate,
                ocr_confidence, flags
            ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        '''
        try:
            ...
            params = (
                ...
                vd.get('class_id'), vd.get('confidence'),
            ...
            with self.lock, self._get_sqlite_connection() as conn:
                conn.execute(sql, params)
            logger.debug(f"Saved track: Feed={params[0]},Track={params[1]},Time={params[2]:.2f}")
            return True
        except RetryError as e: # Tenacity: all retries failed
            logger.error(f"DB save_vehicle_data retries failed: {e}. TrackID: {vd.get('track_id')}")
            return False
        except sqlite3.Error as e: # Catch specific DB errors
            logger.error(f"DB error saving vehicle: {e} - TrackID: {vd.get('track_id')}", exc_info=True)
            if not isinstance(e, sqlite3.OperationalError): # Re-raise if not operational for retry
                raise DatabaseError(f"Failed save vehicle: {e}") from e
            return False # For OperationalError after retries
        except Exception as e: # Catch any other unexpected errors
            logger.error(f"Unexpected error saving vehicle: {e} - TrackID: {vd.get('track_id')}", exc_info=True)
            raise DatabaseError(f"Unexpected save vehicle: {e}") from e
    ...
    def get_recent_tracks(self, feed_id: Optional[str] = None, limit: int = 100) -> List[Dict]:
        try:
            with self._get_sqlite_connection() as conn:
                cursor = conn.cursor()
                if feed_id:
                    sql = "SELECT * FROM vehicle_tracks WHERE feed_id=? ORDER BY timestamp DESC LIMIT ?"
                    params = (feed_id, limit)
                else:
                    sql = "SELECT * FROM vehicle_tracks ORDER BY timestamp DESC LIMIT ?"
                    params = (limit,)
                cursor.execute(sql, params)
                return [dict(row) for row in cursor.fetchall()]
        except sqlite3.Error as e:
            logger.error(f"DB error get recent tracks (feed={feed_id}): {e}", exc_info=True)
            return []
    ...
    def get_track_history(self, feed_id: str, track_id: int, limit: int = 50) -> List[Dict]:
        try:
            with self._get_sqlite_connection() as conn:
                cursor = conn.cursor()
                sql = "SELECT * FROM vehicle_tracks WHERE feed_id=? AND track_id=? ORDER BY timestamp DESC LIMIT ?"
                cursor.execute(sql, (feed_id, track_id, limit))
                # Return in chronological order for history
                return [dict(row) for row in reversed(cursor.fetchall())]
        except sqlite3.Error as e:
            logger.error(f"DB error get track history (feed={feed_id},track={track_id}): {e}", exc_info=True)
            return []

    @lru_cache(maxsize=4) # Consider cache TTL if data freshness is critical
    def get_vehicle_stats(self, time_window_secs: int = 300) -> Dict:
        try:
            with self._get_sqlite_connection() as conn:
                cursor = conn.cursor()
                min_ts = time.time() - time_window_secs
                sql = """
                    SELECT COUNT(*) as total_vehicles, AVG(speed) as average_speed_kmh,
                           SUM(CASE WHEN speed < ? THEN 1 ELSE 0 END) as stopped_vehicles
                    FROM vehicle_tracks WHERE timestamp > ?
                """
                cursor.execute(sql, (DEFAULT_CONFIG['stopped_speed_threshold_kmh'], min_ts))
                res = cursor.fetchone()
                stats = dict(res) if res else {'total_vehicles': 0, 'average_speed_kmh': 0.0, 'stopped_vehicles': 0}
                # Ensure all keys exist even if query returns None (e.g., no data)
            ...
            logger.error(f"DB error get vehicle stats: {e}", exc_info=True)
            return {'total_vehicles': 0, 'average_speed_kmh': 0.0, 'stopped_vehicles': 0} # Default error response

    @lru_cache(maxsize=4) # Consider cache TTL
    def get_vehicle_counts_by_type(self, time_window_secs: int = 300) -> Dict[str, int]:
        try:
            with self._get_sqlite_connection() as conn:
                cursor = conn.cursor()
                min_ts = time.time() - time_window_secs
                # Get the latest class_id for each track_id within the time window
                sql = """
                    WITH LatestTrackInfo AS (
                        SELECT feed_id, track_id, class_id, MAX(timestamp) as max_ts
                        FROM vehicle_tracks
                        WHERE timestamp > ?
                        GROUP BY feed_id, track_id
                    )
                    SELECT class_id, COUNT(DISTINCT track_id) as count
                    FROM LatestTrackInfo
                    GROUP BY class_id
                """
                cursor.execute(sql, (min_ts,))
                results = cursor.fetchall()
                type_map = TrafficMonitor.vehicle_type_map # Assuming TrafficMonitor is accessible
                counts = {name: 0 for name in type_map.values()}
                counts['unknown'] = 0 # Initialize all known types and unknown
                for row in results:
                    type_name = type_map.get(row['class_id'], 'unknown')
                    counts[type_name] = row['count']
                return counts
        except sqlite3.Error as e:
            logger.error(f"DB error get vehicle counts by type: {e}", exc_info=True)
            return {name: 0 for name in TrafficMonitor.vehicle_type_map.values()} | {'unknown':0}
    ...
    def get_alerts_filtered(self, filters: Dict, limit: int = 100, offset: int = 0) -> List[Dict]:
        try:
            with self._get_sqlite_connection() as conn:
                ...
                params: List[Any] = []
                conditions: List[str] = []
                allowed_filters = {"severity", "feed_id", "acknowledged", "start_time", "end_time", "search"}
                ...
                for key, value in filters.items():
                    ...
                    elif key in ["start_time", "end_time"] and isinstance(value, (int, float, str)):
                        # Assuming timestamps might come as ISO strings from API, convert if needed
                        # For simplicity, this example assumes they are already unix timestamps if numeric
                        # Or direct ISO strings if they are strings (SQLite can handle ISO strings for comparison)
                        op = ">=" if key == "start_time" else "<="
                        conditions.append(f"timestamp {op} ?")
                        params.append(value)
                    elif key in ["severity", "feed_id"]:
                        conditions.append(f"{key}=?")
                        params.append(value)

                if conditions:
                    base_q += " AND " + " AND ".join(conditions)
                query_sql = f"{base_q} ORDER BY timestamp DESC LIMIT ? OFFSET ?"
                params.extend([limit, offset])
                cursor.execute(query_sql, params)
                return [dict(row) for row in cursor.fetchall()]
        except sqlite3.Error as e:
            logger.error(f"DB error in get_alerts_filtered: {e}", exc_info=True)
            return []
    ...
    def save_alert(self, severity: str, feed_id: str, message: str, details: Optional[str] = None) -> bool:
        ...
        try:
            ...
            logger.error(f"DB save_alert retries failed: {e}.")
            return False
        except sqlite3.Error as e:
            logger.error(f"DB error saving alert: {e}", exc_info=True)
            if not isinstance(e, sqlite3.OperationalError):
                raise DatabaseError(f"Failed save alert: {e}") from e
            return False
        except Exception as e:
            logger.error(f"Unexpected error saving alert: {e}", exc_info=True)
            raise DatabaseError(f"Unexpected save alert: {e}") from e
    ...
    def acknowledge_alert(self, alert_id: int, acknowledge: bool = True) -> bool:
        ...
        try:
            ...
            with self.lock, self._get_sqlite_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(sql, (ack_val, alert_id))
                conn.commit() # Ensure commit for UPDATE
                if cursor.rowcount == 0:
                    logger.warning(f"Alert ID {alert_id} not found for acknowledgement.")
                    return False
            logger.info(f"Alert ID {alert_id} acknowledgement status set to {acknowledge}.")
            return True
        except RetryError as e:
            logger.error(f"DB acknowledge_alert retries failed: {e}.")
            return False
        except sqlite3.Error as e:
            logger.error(f"DB error acknowledging alert {alert_id}: {e}", exc_info=True)
            if not isinstance(e, sqlite3.OperationalError):
                raise DatabaseError(f"Failed ack alert: {e}") from e
            return False
        except Exception as e:
            logger.error(f"Unexpected error ack alert {alert_id}: {e}", exc_info=True)
            raise DatabaseError(f"Unexpected ack alert: {e}") from e

    @retry(
        wait=wait_exponential(multiplier=0.2, min=0.2, max=3),
        stop=stop_after_attempt(3),
        retry=retry_if_exception_type(Exception) # Generic retry for Mongo for now
    )
    def save_raw_traffic_data_mongo(self, data: Dict) -> bool:
        ...
        try:
            ...
            return True
        except Exception as e: # Catch pymongo errors specifically if possible
    ...
    def get_raw_traffic_data_mongo(
        self, query: Dict, limit: int = 1000,
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not config_file_path.exists():
            ...
            if alt_config_path1.exists():
                config_file_path = alt_config_path1
            elif alt_config_path2.exists():
                config_file_path = alt_config_path2.resolve()
        ...
        print(
            f"Database path: {config_data.get('database',{}).get('db_path')}")
        print(
            f"Model path: {config_data.get('vehicle_detection',{}).get('model_path')}")
        gemini_key = config_data.get('ocr_engine', {}).get('gemini_api_key')
        print(
            f"Gemini Key Set: {'Yes' if gemini_key and gemini_key.strip() else 'No'}")
        ...
        for alert_item in alerts:
            print(
                f"  ID:{alert_item['id']} Time:{time.strftime('%H:%M:%S', time.localtime(alert_item['timestamp']))} Sev:{alert_item['severity']}")
        db_manager.close()
        ...
        if gemini_key and gemini_key.strip():
            ...
            cv2.putText(dummy_roi, "TEST", (50, 70),
                        cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 3)
            print(
                "Attempting OCR on a dummy image with Gemini (expect empty or error if key invalid)...")
            ...
            lp_preprocessor = LicensePlatePreprocessor(
                config_data)  # Init without Gemini
            dummy_roi_tess = np.zeros((100, 300, 3), dtype=np.uint8)
            cv2.rectangle(dummy_roi_tess, (10, 10), (290, 90),
                          (50, 50, 50), -1)  # Dark background
            cv2.putText(dummy_roi_tess, "TST123", (40, 70), cv2.FONT_HERSHEY_SIMPLEX,
                        2, (220, 220, 220), 5, cv2.LINE_AA)  # Light text
            print("Attempting OCR on a dummy image with Tesseract (expect TST123 or similar if Tesseract installed)...")
            ocr_text_tesseract = lp_preprocessor.preprocess_and_ocr(
                dummy_roi_tess)
