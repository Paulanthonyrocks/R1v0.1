...
def get_feed_manager() -> "FeedManager":
    ...
    try:
        ...
        if (
            not hasattr(services, "feed_manager_instance")
            or services.feed_manager_instance is None
        ):
            logger.error(
                "FeedManager not initialized. Call initialize_services() first."
            )
            raise RuntimeError(
...
def get_analytics_service() -> "AnalyticsService":
    ...
    try:
        ...
        analytics_instance = getattr(
            services, "analytics_service_instance", None
        ) or getattr(services, "_analytics_service_instance", None)

        if analytics_instance is None:
            logger.error(
                "AnalyticsService not initialized. Call initialize_services() first."
            )
            raise RuntimeError(
...
def get_connection_manager() -> "ConnectionManager":
    ...
    try:
        ...
        if (
            not hasattr(services, "connection_manager_instance")
            or services.connection_manager_instance is None
        ):
            logger.error(
                "ConnectionManager not initialized. Call initialize_services() first."
            )
            raise RuntimeError(
...
def get_service(service_name: str):
    ...
    if service_name not in service_getters:
        raise ValueError(
            f"Unknown service: {service_name}. Available: {list(service_getters.keys())}"
        )
