  ...
  useEffect(() => {
    if (metrics) {
      setHistory(prev => {
        ...
        const newEntry = {
          timestamp: now,
          congestion: typeof metrics.total_flow === 'number' ? metrics.total_flow : 0,
      ...
      <div className="bg-lcd-bg text-lcd-text font-lcd flex flex-col min-h-screen w-full">
        ...
        <div className="p-4 grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 relative">
          ...
          <div className="col-span-full grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            <StatCard title="Vehicles Detected" value={String(metrics?.total_flow ?? '--')} unit="" icon={Activity} change="N/A" changeText="N/A" />
            <StatCard title="Average Speed" value={metrics?.average_speed_kmh ? Number(metrics.average_speed_kmh).toFixed(2) : '--'} unit="km/h" icon={Zap} change="N/A" changeText="N/A" />
